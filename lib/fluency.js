...
const OKGFluency = (function () {
  ...
  const BACKWARD_DECAY = 0.85;     // per hop: implied prereq fluency = score * 0.85^d
  const FORWARD_CAP_WEIGHT = 0.9;  // successor capped at 90% of min hard-prereq fluency
  const MAX_PROPAGATION_DEPTH = 6; // don't propagate beyond 6 hops
  ...
  function propagate(graph) {
    ...
    for (var i = 0; i < directIds.length; i++) {
      ...
      for (var p = 0; p < prereqs.length; p++) {
        if (!visited[prereqs[p]]) {
          queue.push({id: prereqs[p], depth: 1});
          visited[prereqs[p]] = true;
        }
      }
      ...
      while (queue.length > 0) {
        ...
        var implied = Math.round(startScore * Math.pow(BACKWARD_DECAY, item.depth));
        ...
        for (var a = 0; a < ancestors.length; a++) {
          if (!visited[ancestors[a]]) {
            queue.push({id: ancestors[a], depth: item.depth + 1});
            visited[ancestors[a]] = true;
          }
        }
      }
    }
  ...
  function topoSort(graph) {
    ...
    for (var i = 0; i < ids.length; i++) {
      ...
      for (var s = 0; s < succs.length; s++) {
        inDegree[succs[s]] = (inDegree[succs[s]] || 0) + 1;
      }
    }
    ...
    while (queue.length > 0) {
      ...
      for (var c = 0; c < children.length; c++) {
        inDegree[children[c]]--;
        if (inDegree[children[c]] === 0) queue.push(children[c]);
      }
    }
   ...
   */
  function findFrontier(graph, effective) {
    var frontier = [];
    ...
    for (var id in graph) {
      ...
      if (prereqs.length === 0) {
        // Root topics with no prereqs: frontier if not yet learned
        if (ownScore < FRONTIER_SELF_THRESHOLD) {
          frontier.push({id: id, readiness: 100, ownScore: ownScore});
        }
        continue;
      }

      // Average prereq fluency
      var sum = 0;
      var count = 0;
      for (var p = 0; p < prereqs.length; p++) {
        sum += (effective[prereqs[p]] || 0);
        count++;
      }
      var avgPrereq = count > 0 ? sum / count : 0;

      if (avgPrereq >= FRONTIER_PREREQ_THRESHOLD) {
        frontier.push({
          id: id,
          readiness: avgPrereq - ownScore,  // higher gap = more ready
          ownScore: ownScore,
        });
      }
    }
  ...
  function isFrontier(topicId, graph, effective) {
    ...
    var prereqs = node.prereqs || [];
    if (prereqs.length === 0) return ownScore < FRONTIER_SELF_THRESHOLD;

    var sum = 0;
    for (var p = 0; p < prereqs.length; p++) {
      sum += (effective[prereqs[p]] || 0);
    }
    return (sum / prereqs.length) >= FRONTIER_PREREQ_THRESHOLD;
  }
  ...
  function postAssessmentInference(domainPerformance, topicIndex) {
    ...
    function applyFloor(topicIds, floor, inferConf) {
      if (!topicIds) return;
      for (var i = 0; i < topicIds.length; i++) {
        var tid = topicIds[i];
        var current = scores[tid] || 0;
        var currentConf = conf[tid] || 0;
        if (current < floor && currentConf < 0.6) {
          scores[tid] = floor;
    ...
    if (overallTier >= 1) {
      ...
      for (var gd in topicIndex) {
        ...
        for (var si2 = 0; si2 < overallTier; si2++) {
          ...
          var tFloor = Math.min(95, 68 + dist * 7);
          var tConf = Math.min(0.45, 0.12 + dist * 0.08);
          applyFloor(topicIndex[gd][STAGE_ORDER[si2]], tFloor, tConf);
        }

        // At the tier stage itself — modest floor in general-ed domains
        if (overallTier < STAGE_ORDER.length) {
          applyFloor(topicIndex[gd][STAGE_ORDER[overallTier]], 55, 0.1);
        }
      }
    }
    ...
    for (var domain in domainPerformance) {
      ...
      // Fill stages BELOW the ceiling
      for (var belowIdx = 0; belowIdx < ceilingIdx; belowIdx++) {
        var distance = ceilingIdx - belowIdx;
        var floor = Math.min(92, 72 + distance * 5);
        var inferConf = Math.min(0.5, 0.2 + distance * 0.08);
        applyFloor(topicIndex[domain][STAGE_ORDER[belowIdx]], floor, inferConf);
      }
