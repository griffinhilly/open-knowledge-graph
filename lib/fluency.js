...
const OKGFluency = (function () {
  ...
  function propagate(graph) {
    ...
    for (var t = 0; t < topo.length; t++) {
      ...
      for (var pr = 0; pr < node.prereqs.length; pr++) {
        ...
        // Origin layer: kind:capacity prereqs are assumed-known and must NEVER cap a successor.
        // A capacity seeded/assessed to 100 would otherwise impose a 0.9 forward cap that cascades
        // 0.9^depth down every chain rooted at a pre-formal topic (see plans/origin-layer-spec.md
        // sec 0.2 — the verified fluency regression the premortem caught).
        if (pid && graph[pid] && graph[pid].kind === 'capacity') continue;
  ...
  function computePathToGoal(graph, scores, goalId) {
    ...
    // Origin layer: kind:capacity nodes are assumed-known for learners and are never surfaced as
    // path steps — this is how "assumed-known" is realized (see plans/origin-layer-spec.md sec 0.1/0.2).
    var unmastered = [];
    for (var id in needed) {
      if (graph[id] && graph[id].kind === 'capacity') continue;
