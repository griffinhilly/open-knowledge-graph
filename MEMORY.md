...
- **Strategy fork RESOLVED Jun 12 (Griffin): distribution track PRIMARY; parent-acquisition PARKED until ~mid-July** (re-entry for August back-to-school push). Key rationale: the parent-track pause was partly seasonal — promoting an educational resource to parents late in the school year felt futile; August school resumption is the natural window. Distribution sequence: B3+A6 → keystone leaderboard (edge-audit gate) → path engine (no-path-fallback gate) → B2 Show HN → .ics card. Pre-launch: README (repo has NONE), analytics, Node-24 action bumps.
