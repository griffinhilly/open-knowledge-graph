...
- question: "A function f: X → X on a complete metric space satisfies d(f(x), f(y)) < d(x, y) for all distinct x, y (it strictly decreases every distance, but without a uniform bound c < 1). The Banach Fixed Point Theorem guarantees it has a unique fixed point."
  type: true-false
  answer: false
