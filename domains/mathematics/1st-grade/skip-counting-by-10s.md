...
```yaml
- question: "Which sequence shows correct skip counting by 10s?"
  ...
  options:
    ...
    - "10, 20, 25, 30"
  answer: 1
  explanation: "When we skip count by 10s, each number is exactly 10 more than the one before it. Only 10, 20, 30, 40 follows this rule — every step adds exactly 10. The other sequences mix in additions of 1 or 5."

- question: "When you skip count by 10s starting from 10, you will eventually say the number 25."
  type: true-false
  answer: false
  explanation: "Skip counting by 10s produces 10, 20, 30, 40, 50, and so on. You always add 10, so you always land on numbers whose ones digit is 0. The number 25 has a ones digit of 5, so it never appears in this sequence."

- question: "What do you notice about the ones digit (the digit in the ones place) of every number when you skip count by 10s starting at 10?"
  type: short-answer
  answer: "The ones digit is always 0. Every number in the sequence — 10, 20, 30, 40, 50 — ends in 0."
  explanation: "This is the key pattern that makes skip counting by 10s so useful. When you add 10, only the tens digit increases by 1; the ones digit stays the same. Starting at 10 (ones digit: 0), it stays 0 every time. This is one of the earliest glimpses of how our place-value number system works."
```
