...
```yaml
- question: "If you fold a piece of paper into two equal pieces, what do you call each piece?"
  type: multiple-choice
  options: ["A whole", "A half", "A quarter", "A third"]
  answer: 1
  explanation: "When something is divided into 2 equal parts, each part is called a half. The key word is 'equal' — both pieces must be the same size for each one to be a half."

- question: "If you cut a sandwich into two pieces and one piece is bigger than the other, each piece is called a half."
  type: true-false
  answer: false
  explanation: "Halves must be equal in size. If the two pieces are different sizes, neither one is truly a half. Fractions only name the parts correctly when the whole is divided into equal pieces."

- question: "A pizza is cut into 4 equal slices. What do you call each slice?"
  type: short-answer
  answer: "A quarter, or one-fourth"
  explanation: "When a whole is divided into 4 equal parts, each part is called a quarter or one-fourth. The number of equal pieces tells us the name of each fraction part."
```
