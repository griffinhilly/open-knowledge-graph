...
- Students sometimes divide in the wrong order (dividing the single unit by the quantity instead of the other way around).
- Forgetting to include units, which makes the rate meaningless.
- Confusing a rate with a ratio — all rates are ratios, but not all ratios are rates (rates involve different units).
