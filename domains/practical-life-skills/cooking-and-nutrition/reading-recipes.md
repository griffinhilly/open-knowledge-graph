...
- Recipes are rigid formulas — most are templates that tolerate reasonable substitution.
- 'Prep time' listed in recipes includes all the chopping and measuring, so the total time is often longer than it appears.

## Questions

```yaml
- question: "What is the primary purpose of reading a recipe completely before starting to cook?"
  type: multiple-choice
  options: ["To memorize the ingredients list", "To estimate how expensive the dish will be", "To avoid mid-cook surprises like missing ingredients or steps that require advance prep", "To check whether the recipe is worth making"]
  answer: 2
...
- question: "A recipe calls for 3/4 cup of flour, but you want to make a double batch. How much flour do you need, and what math concept helps you scale recipes?"
  type: short-answer
  answer: "1.5 cups (3/4 × 2 = 6/4 = 3/2 = 1.5). Scaling recipes uses proportional reasoning — multiplying all ingredient quantities by the same factor."
  explanation: "Recipes are proportional relationships: the ratio between ingredients stays constant regardless of batch size. To scale up by 2×, multiply every quantity by 2. Fractions appear constantly in recipe measurements (1/4 tsp, 2/3 cup), so comfort with fraction multiplication directly enables confident recipe scaling."
```
