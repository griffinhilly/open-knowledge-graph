...
- Meal planning requires identical meals each day — the goal is having components available, not predetermined identical plates.
- Meal planning only saves money for large families — even a single person reduces waste and impulse spending significantly.

## Questions

```yaml
- question: "Which approach best describes effective meal planning?"
  type: multiple-choice
  options:
    - "Cooking the same fixed breakfast, lunch, and dinner every day of the week"
    - "Preparing a set of flexible components (grains, proteins, vegetables) that can be combined into different meals"
    - "Planning every meal from scratch with a unique new recipe each day"
    - "Only planning dinners and leaving all other meals to be decided spontaneously"
  answer: 1
...
- question: "What is 'skeleton planning,' and what practical advantage does it offer over fully scripted meal plans?"
  type: short-answer
  answer: "Skeleton planning means committing to the main structural elements of meals — proteins and grains — in advance, while leaving vegetables flexible to incorporate whatever is on sale or in season. The advantage is cost savings from seasonal produce and reduced rigidity, so the plan survives real-world variation (a sale, an unexpected event) without collapsing entirely."
  explanation: "Fully scripted plans break down when one ingredient is unavailable or expensive. A skeleton plan preserves the decision-reducing benefit of planning while allowing tactical flexibility at the store, which is especially useful when building a grocery list that balances nutrition and budget."
```
