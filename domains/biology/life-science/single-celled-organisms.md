...
- Thinking single-celled organisms are not really alive because they are too small.
- Believing all bacteria are harmful — most bacteria are harmless or beneficial (digestion, decomposition).
- Assuming single-celled organisms are "simple" and unimportant — they drive global nutrient cycles and outnumber all other organisms combined.

## Questions

```yaml
- question: "What makes a single-celled organism different from a multi-celled organism?"
  type: multiple-choice
  options: ["It cannot reproduce", "It is made of only one cell that does everything", "It does not need energy", "It is always harmful"]
  answer: 1
...
- question: "Explain how a single-celled organism like an amoeba can survive without specialized organs."
  type: short-answer
  answer: "An amoeba's single cell performs all necessary life functions on its own. It takes in food by surrounding particles with its cell membrane, breaks down food for energy, removes waste, and reproduces by splitting in two. It does not need separate organs because one cell handles everything."
  explanation: "This highlights the self-sufficiency of cells. Organs are collections of specialized cells in multi-celled organisms, but a single cell already contains all the molecular machinery needed for life. Specialization is a strategy for efficiency in larger organisms, not a requirement for life itself."
```
