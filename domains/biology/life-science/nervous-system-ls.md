...
- Thinking the brain is the only part of the nervous system — the spinal cord and peripheral nerves are equally essential.
- Believing we only have five senses — we also have senses for balance, body position, temperature, pain, and more.
- Thinking nerve signals are instant — they are very fast but not instantaneous; reaction time demonstrates this.

## Questions

```yaml
- question: "What is the correct path of a nerve signal when you touch something hot?"
  type: multiple-choice
  options: ["Brain → spinal cord → hand", "Hand → sensory nerve → brain → motor nerve → muscles", "Muscles → brain → sensory nerve → hand", "Brain → muscles → sensory nerve → spinal cord"]
  answer: 1
...
- question: "Explain what a reflex is and why it is faster than a normal response."
  type: short-answer
  answer: "A reflex is an automatic, rapid response to a stimulus that does not require the brain to make a decision. It is faster because the signal travels only to the spinal cord and back, skipping the brain. For example, when a doctor taps your knee, the signal goes to the spinal cord, which immediately sends a signal back to the leg muscle to kick — no thinking required."
  explanation: "Reflexes are a survival mechanism — they protect the body from harm before the brain even registers what happened. The shorter signal path (sensory nerve → spinal cord → motor nerve, bypassing the brain) is why reflexes are faster than deliberate responses."
```
