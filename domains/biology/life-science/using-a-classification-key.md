---
id: using-a-classification-key
title: Using a Classification Key
domain: biology
course: life-science
prerequisites:
...
- Thinking there is only one correct way to build a dichotomous key — many valid keys can be constructed for the same set of organisms; the questions just need to reliably separate them.
- Believing a key tells you everything about an organism — it only identifies the organism; you need other resources for details about its biology.
- Expecting every question to be about visible features — some keys use habitat, behavior, or geographic location as distinguishing criteria.

## Questions

```yaml
- question: "What does the word 'dichotomous' mean in the context of a classification key?"
  type: multiple-choice
  options: ["It means the key uses color-coding", "It means every question has exactly two choices", "It means the key works only for animals", "It means the key was created by a computer"]
  answer: 1
...
- question: "Describe the steps you would follow to identify an unknown leaf using a dichotomous key."
  type: short-answer
  answer: "Start at step 1 and read both options. Observe the leaf and choose the option that matches (for example, 'Leaf has smooth edges' vs. 'Leaf has toothed edges'). Follow the instruction next to your choice — it will either send you to another step or give you the identification. Continue choosing between two options at each step until you reach a final identification."
  explanation: "The key strength of a dichotomous key is its step-by-step structure. You never need to know everything about the organism at once — each step narrows the possibilities by one distinguishing feature. Even someone unfamiliar with the organisms can use a well-constructed key to arrive at a correct identification."
```
