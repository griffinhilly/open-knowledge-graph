...
- Thinking energy is recycled in ecosystems like nutrients — energy flows in one direction and is eventually lost as heat; it is not recycled.
- Believing top predators get the most energy — they get the least, which is why they are the rarest organisms in an ecosystem.
- Assuming the 10% rule means 90% of food is wasted — the 90% is used by each organism for its own life processes; it is not wasted, just converted to heat.

## Questions

```yaml
- question: "Why are there typically fewer top predators than herbivores in an ecosystem?"
  type: multiple-choice
  options: ["Because predators are naturally rarer due to genetics", "Because 90% of energy is lost at each level, so less energy is available to support organisms at higher levels", "Because herbivores reproduce faster than predators", "Because predators need less food"]
  answer: 1
  explanation: "At each level of the food chain, about 90% of the energy is used for life processes and lost as heat. Only about 10% is stored in body tissue and available to the next level. This means each successive level has far less energy to support its organisms. A large base of producers supports a smaller number of herbivores, which supports an even smaller number of predators."

- question: "Energy is recycled in ecosystems just like nutrients are."
  type: true-false
  answer: false
  explanation: "Nutrients (like nitrogen and phosphorus) are recycled — decomposers return them to the soil, and producers take them up again. But energy is NOT recycled. It enters the ecosystem as sunlight, is converted to chemical energy by producers, and is gradually lost as heat at each trophic level. The ecosystem must receive a continuous supply of new energy from the sun."

- question: "If a field of grass captures 10,000 kcal of energy from the sun, approximately how much energy is available to secondary consumers (like snakes that eat mice)?"
  type: short-answer
  answer: "About 100 kcal. The grass passes about 10% (1,000 kcal) to primary consumers (mice). The mice pass about 10% of that (100 kcal) to secondary consumers (snakes). Each level loses about 90% of the energy as heat."
  explanation: "Applying the 10% rule across two transfers: 10,000 → 1,000 → 100. This dramatic reduction explains why ecosystems are shaped like pyramids — wide at the base (many producers) and narrow at the top (few top predators)."
```
