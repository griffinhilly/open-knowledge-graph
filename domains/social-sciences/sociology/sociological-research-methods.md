...
- Qualitative research is not less rigorous than quantitative; it uses different standards of rigor (transferability, credibility, reflexivity).
- A larger sample does not automatically produce valid results — question wording, sampling frame, and response bias matter enormously.

## Questions

```yaml
- question: "A sociologist wants to understand why employees at one company leave their jobs at higher rates than at a comparable company. Which method is best suited to generating initial hypotheses about this?"
  type: multiple-choice
  options:
    - "A large-scale national survey of worker satisfaction"
    - "In-depth interviews with current and former employees at both companies"
    - "Secondary analysis of industry-wide turnover statistics"
    - "A randomized controlled experiment assigning workers to different managers"
  answer: 1
...
- question: "A researcher conducts an ethnographic study of a single high school for two years. A critic argues the findings cannot be generalized. How should the ethnographer respond?"
  type: short-answer
  answer: "Ethnographic research aims for transferability, not statistical generalizability. The goal is thick description — detailed contextual findings that readers can assess for applicability to their own situations — not claims that the results hold across all high schools in a population."
  explanation: "Qualitative and quantitative research operate under different logics of rigor. Generalizability in the statistical sense requires representative sampling and is appropriate for quantitative research. Qualitative research instead seeks credibility (are the findings trustworthy?), transferability (can readers judge applicability to other contexts?), and reflexivity (did the researcher account for their own position?). Neither standard is superior — they answer different kinds of questions."
```
