...
- Close reading is not the same as summarizing — it asks 'how' and 'why', not 'what happens'.
- Students often assume their first interpretation is obvious rather than argued; every claim needs textual warrant.
- Close reading doesn't mean ignoring context; historical or biographical context can sharpen interpretation.

## Questions

```yaml
- question: "A student reads Hamlet's 'To be or not to be' soliloquy and notes: 'Shakespeare uses the word 'slings' metaphorically — slings are weapons that hurl stones, implying that misfortune is an aggressive, violent force attacking the speaker.' This is an example of:"
  type: multiple-choice
  options: ["Summarizing the plot of the scene", "Moving from observation to interpretation using textual evidence", "Ignoring the text in favor of biographical context", "Describing what happens rather than how or why"]
  answer: 1
...
- question: "What is the difference between an observation and an interpretation in close reading?"
  type: short-answer
  answer: "An observation identifies a specific textual feature (e.g., 'the sentences get shorter here'). An interpretation argues what that feature means or does (e.g., 'the shorter sentences create a sense of mounting panic'). Close reading requires both: observations are the evidence, interpretations are the claims."
  explanation: "Many readers stop at observation without advancing to interpretation, which produces description rather than argument. The move from 'what is there' to 'what it means or does' is the analytical core of close reading. Every interpretive claim should be traceable back to specific textual observations."
```
