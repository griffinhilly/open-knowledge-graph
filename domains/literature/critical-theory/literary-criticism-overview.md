...
title: Literary Criticism as a Discipline
domain: literature
...
- Literary criticism is not the same as literary evaluation (saying whether a work is good); it is analysis of how and why texts mean.
- No single critical school is 'correct'; they address different questions.
- 'Theory' does not make reading more difficult by adding jargon—it makes previously invisible assumptions explicit and debatable.

## Questions

```yaml
- question: "A student reads a poem and concludes that it is 'beautiful' and 'moving.' A literary critic analyzes the same poem and argues that its use of the pastoral mode naturalizes aristocratic privilege by hiding labor. Which statement best describes the difference between these two responses?"
  type: multiple-choice
  options: ["The critic is correct and the student is wrong", "The student is doing evaluation while the critic is doing criticism", "The critic is imposing an ideology onto a text that doesn't support it", "Both responses are equally valid expressions of personal taste"]
  answer: 1
  explanation: "Literary criticism is systematic analysis and interpretation using theoretical frameworks, not personal aesthetic evaluation. The student is evaluating — judging the poem's quality based on personal response. The critic is performing criticism — analyzing how the text produces meaning and what ideological work it does. Neither response cancels the other; they answer different questions. Criticism is not about whether a work is good but about how and why it means what it means."

- question: "Applying a Marxist critical lens to a novel automatically excludes the possibility of also applying a feminist lens to the same text."
  type: true-false
  answer: false
  explanation: "Critical schools are not mutually exclusive frameworks where choosing one rules out others. They ask different questions and illuminate different features of the same text. A Marxist reading might analyze class relations and economic structures; a feminist reading might analyze gender power dynamics. Both can be applied to the same text, and the readings may even inform each other — for example, examining how class and gender intersect. The misconception that schools 'compete' treats them as theories of what the text 'really' means rather than as lenses that reveal different aspects."

- question: "What does it mean to say that a critical interpretation is an argument rather than a fact?"
  type: short-answer
  answer: "It means an interpretation must be supported with textual and theoretical evidence, can be contested, and is evaluated by how well it accounts for the evidence — not by whether it matches the author's intent or some fixed correct meaning."
  explanation: "Calling an interpretation an argument distinguishes literary criticism from mere summary or personal reaction. Arguments have premises, use evidence (quotations, textual details, theoretical frameworks), and can be challenged with counter-evidence. Two critics reading the same text can reach opposite interpretations, and the debate is adjudicated by evidence and reasoning — not by deference to the author's intention (the intentional fallacy) or to majority opinion. This is what makes criticism a disciplined intellectual activity."
```
