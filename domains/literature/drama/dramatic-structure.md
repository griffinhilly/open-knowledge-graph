...
- Students often confuse the climax with the most emotionally intense scene rather than the turning point in the protagonist's fortunes.
- 'Resolution' does not mean all conflicts are resolved — the denouement simply shows consequences after the climax.

## Questions

```yaml
- question: "In Freytag's Pyramid, which element marks the turning point at which the protagonist's fortune decisively shifts — the point of highest tension from which the outcome becomes inevitable?"
  type: multiple-choice
  options: ["The inciting incident", "The crisis (or climax)", "The falling action", "The denouement"]
  answer: 1
...
- question: "How does dramatic structure in plays differ from narrative structure in prose fiction in ways that affect how playwrights must construct their scenes?"
  type: short-answer
  answer: "Drama conveys its arc almost entirely through live dialogue and action with no narrator, compressed into theatrical time, with scene changes that interrupt flow. This forces every scene to actively advance conflict — there is no room for exposition-heavy passages or internal monologue that prose can use to develop the arc."
  explanation: "A novelist can pause action to describe backstory or interiority. A playwright must embed all of that in dialogue and visible action, and theatrical conventions (limited stage time, no pause between scenes in performance) mean structural choices are more consequential. A scene that stalls forward momentum is much more damaging in drama than in a novel because the audience experiences it in real time with no option to skim."
```
