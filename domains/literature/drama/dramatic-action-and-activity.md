...
title: Dramatic Action Versus Physical Activity
domain: literature
course: drama
prerequisites:
...
```yaml
- question: "A scene shows a character spending five minutes searching an office for a document, failing to find it, and then leaving. Is this dramatic action?"
  type: multiple-choice
  options:
    - "Yes — the character has taken physical actions with clear goals"
    - "Only if the audience learns something new from watching the search"
    - "No — unless the failure to find the document changes the situation and affects what follows"
    - "Yes — any goal-directed behavior counts as dramatic action"
  answer: 2
  explanation: "The test of dramatic action is whether something changes in the dramatic situation — not whether the character has goals or does things. The search is dramatic action only if its outcome (success or failure) makes the next moment different: the character now knows the document is gone, must revise their plan, and takes a different path as a result. If the character walks out and the plot continues identically to how it would have if the scene hadn't occurred, the scene contained no dramatic action — regardless of how much physical activity filled it."

- question: "Why is 'character is revealed through choice under pressure' central to the concept of dramatic action?"
  type: short-answer
  answer: "Dramatic action requires genuine stakes — the cost of acting must be real. When a character chooses under pressure (to confess, to refuse, to betray), the choice is irreversible and costly, which makes it revealing in a way activity is not. Activity shows type (anxious, controlled, cautious); dramatic action shows character in the full sense because it shows what a person does when the outcome matters. The most significant dramatic action is always made by the character for whom it costs the most."
  explanation: "This connects the dramaturgy concept back to characterization methodology: physical description and habitual behavior establish type, but the central characterization method in drama is action under pressure. The playwright creates situations in which characters must choose, and what they choose — not what they think or feel — constitutes who they are. Hamlet's famous delay is dramatically significant precisely because his *inaction* is itself a choice, and the cost of that choice accumulates throughout the play."
```
