...
title: Rhythmic Development and Variation
domain: music
course: composition
prerequisites:
...
```yaml
- question: "A composer has a two-measure rhythmic motive with the pattern: quarter, eighth, eighth, half. She applies augmentation (doubling all values). What is the result?"
  type: short-answer
  answer: "The augmented pattern is: half, quarter, quarter, whole. Each note value is doubled. The original two-measure motive now spans four measures. The proportional relationships between the notes are preserved."
  explanation: "Augmentation doubles every note value proportionally. Quarter becomes half, eighth becomes quarter, half becomes whole. The figure's internal rhythmic relationships (short-short-long, in this case) remain identical — only its time scale changes. This is why augmented material remains recognizable even as it expands to occupy more formal space."

- question: "Metric displacement takes a rhythmic figure and does what to it?"
  type: multiple-choice
  options:
    - "Inverts the order of its note values (retrograde rhythm)"
    - "Shifts the figure's position within the measure, placing its accents off the expected beat"
    - "Doubles all note values while preserving proportions"
    - "Removes notes from the figure to create a simpler variant"
  answer: 1
  explanation: "Metric displacement moves the onset of a rhythmic figure to a different position in the metric grid. A figure that originally fell on beat 1 might be displaced to begin on the 'and' of 2, so its accents now conflict with the underlying pulse. The figure itself is unchanged — same note values, same proportions — but its relationship to the meter shifts, creating cross-rhythmic tension."
```
