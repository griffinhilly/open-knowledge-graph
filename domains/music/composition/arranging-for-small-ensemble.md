...
title: Arranging for Small Ensemble
domain: music
course: composition
prerequisites:
...
- Transcription (note-for-note transfer) is not arrangement — effective arranging requires active decisions to make material idiomatic for the new medium.
- Ensemble balance is not automatic from notation; dynamic markings and register choices must compensate for the acoustic differences between instruments.

## Questions

```yaml
- question: "When adapting a piano piece for string trio, why might the arranger redistribute voices rather than copying the left-hand part to cello and the right-hand part to violin and viola?"
  type: multiple-choice
  options: ["String players cannot read both treble and bass clef", "Piano voicings often span ranges or require simultaneous notes that are physically impossible or awkward on individual string instruments", "String instruments always play louder than the piano writing implies", "The cello cannot play in the same register as the left hand"]
  answer: 1
