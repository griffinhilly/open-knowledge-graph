...
title: 'Functional Harmony: Tonic, Subdominant, and Dominant'
domain: music
...
- Treating harmonic function as rigid rules rather than tendencies: vii° can occasionally appear in Tonic contexts depending on voice leading.
- Conflating chord quality (major/minor) with harmonic function — minor chords can serve Dominant or Tonic functions depending on context.

## Questions

```yaml
- question: "In C major, which chord serves the Dominant function?"
  type: multiple-choice
  options: ["IV (F major)", "I (C major)", "ii (D minor)", "V (G major)"]
  answer: 3
...
- question: "Why can the ii chord substitute for the IV chord in a Tonic–Subdominant–Dominant–Tonic progression?"
  type: short-answer
  answer: "Both ii and IV share the same Subdominant function — they create motion away from tonic stability and prepare the Dominant. They also share two common tones (in C major, IV = F-A-C and ii = D-F-A share F and A), so the substitution produces a similar harmonic color with slightly more tension."
  explanation: "Functional harmony is about what a chord does, not which chord it is. Any chord in the Subdominant family (IV or ii) can fulfill the S slot in the T–S–D–T cycle. The common tones between ii and IV make the substitution smooth, and ii tends to feel slightly more active because it has no tonic note, making it a popular choice before a dominant cadence."
```
