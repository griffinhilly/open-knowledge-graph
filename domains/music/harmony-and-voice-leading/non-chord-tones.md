...
- Mistaking a suspension for a chord tone: a suspension is always dissonant against the prevailing chord.
- Confusing a passing tone (moves through) with a neighbor tone (returns to starting pitch).
- Assuming that any dissonant note is an NCT — some dissonances are chord tones of complex harmonies.

## Questions

```yaml
- question: "A melody note on a weak beat steps from G (a chord tone of G major) up to A, then returns by step back to G. What type of non-chord tone is the A?"
  type: multiple-choice
  options: ["Passing tone", "Neighbor tone", "Suspension", "Anticipation"]
  answer: 1
...
- question: "Explain the difference between a passing tone and a neighbor tone."
  type: short-answer
  answer: "A passing tone moves by step between two different chord tones (bridging the gap), while a neighbor tone steps away from a chord tone and returns to that same chord tone."
  explanation: "The key distinction is destination. Passing tones travel from one chord tone to another, filling a melodic gap stepwise. Neighbor tones orbit a single chord tone, creating brief embellishment before returning. This difference gives them different expressive effects: passing tones create forward momentum, while neighbor tones create a gentle oscillation around a stable pitch."
```
