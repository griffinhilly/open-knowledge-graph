...
- Confusing borrowed chords with modulation: borrowing a chord does not change the key, even if it contains notes foreign to the original scale.
- Thinking borrowed chords only come from the parallel minor: Dorian and Mixolydian borrowings are common in pop music.
- Mislabeling borrowed chords as secondary dominants when they do not resolve with dominant-to-tonic motion.

## Questions

```yaml
- question: "Which of the following chords is borrowed from the parallel minor in C major?"
  type: multiple-choice
  options: ["A minor (vi)", "F minor (iv)", "G major (V)", "E minor (iii)"]
  answer: 1
  explanation: "F minor (iv) is built on the fourth scale degree with a minor third, which requires Ab — a note from C minor, not C major. The other options (vi, V, iii) are all diatonic in C major and do not require borrowing. The minor iv is one of the most emotionally distinctive and frequently used borrowed chords."

- question: "Using a borrowed chord from the parallel minor in a major-key piece means you have modulated to the minor key."
  type: true-false
  answer: false
  explanation: "This is the most common misconception about borrowed chords. Borrowing a chord means temporarily importing a chord from the parallel mode without changing the tonal center. The piece remains in the major key throughout. Modulation requires an established shift of tonal center, not just a chromatic chord."

- question: "What makes borrowed chords emotionally effective when used in a major-key context?"
  type: short-answer
  answer: "Borrowed chords introduce flatted scale degrees (especially the b6 and b7) that create a momentary darker, more minor-tinged color without leaving the major key. The contrast between the major-key brightness and the borrowed chord's darkness creates expressive tension."
  explanation: "The power of borrowed chords lies in the contrast they create. In a bright major context, a suddenly minor iv or a bVI chord introduces notes foreign to the scale, creating a shadow or emotional weight. Because the tonal center does not change, the listener experiences the color shift but does not feel lost — the harmony returns to familiar territory."
```
