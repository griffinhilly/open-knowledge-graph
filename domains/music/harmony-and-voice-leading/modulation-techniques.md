...
- Thinking all key changes require a pivot chord: direct and sequential modulation are common and legitimate.
- Misidentifying enharmonic modulation: it requires an actual respelling of the shared chord, not merely moving to a distant key.
- Treating all modulations as equally structurally significant: a passing tonicization and a full modulation to a new key area have very different structural weight.

## Questions

```yaml
- question: "A composer ends a phrase in C major with a perfect authentic cadence, then begins the next phrase in E major with no transitional harmony between the keys. Which modulation technique is this?"
  type: multiple-choice
  options: ["Pivot chord modulation", "Sequential modulation", "Enharmonic modulation", "Direct (phrase) modulation"]
  answer: 3
  explanation: "Direct modulation — also called phrase modulation — simply begins the new key without preparation, typically at a phrase or section boundary. No shared pivot chord is used, and no gradual sequence leads to the new tonal center. The abrupt shift is characteristic and is common at the start of a chorus in pop music or at phrase boundaries in Classical repertoire."

- question: "Enharmonic modulation can be achieved by moving to any distant key, as long as the harmonic distance is large enough."
  type: true-false
  answer: false
  explanation: "Enharmonic modulation specifically requires respelling a chord enharmonically so that it can be heard as a diatonic chord in the new key. Simply moving to a distant key without that reinterpretation is a direct modulation, not an enharmonic one. The respelling is the defining feature: a German augmented sixth chord reinterpreted as a dominant seventh, for example, or a diminished seventh chord reread in a new key."

- question: "Explain how a diminished seventh chord can be used to modulate to multiple different keys."
  type: short-answer
  answer: "A fully diminished seventh chord is symmetrically constructed with three minor thirds stacked, dividing the octave into four equal parts. Any of its four notes can be enharmonically respelled as the leading tone of a different key, making the chord function as the viio7 in up to four different keys. By choosing which respelling to apply, a composer can resolve the chord smoothly into any of those key areas."
  explanation: "The symmetry of the diminished seventh is what gives it this flexibility. Because all four inversions are acoustically identical, the same sonority can be recontextualized harmonically in multiple ways — a technique Romantic composers exploited to modulate to dramatically distant keys."
```
