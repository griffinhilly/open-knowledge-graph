...
- Harmonic and melodic minor are not separate keys — they are variations on the natural minor scale used in different musical situations.
- The melodic minor scale ascending and descending are different; this is intentional and not an error.

## Questions

```yaml
- question: "What distinguishes the harmonic minor scale from the natural minor scale?"
  type: multiple-choice
  options: ["The third scale degree is raised by a half step", "The seventh scale degree is raised by a half step", "The sixth scale degree is lowered by a half step", "The fifth scale degree is raised by a half step"]
  answer: 1
  explanation: "Harmonic minor raises only the seventh scale degree by a half step compared to natural minor. This creates a leading tone — a note a half step below the root with a strong pull to resolve upward. The raised seventh produces the characteristic augmented second interval between scale degrees 6 and 7, which gives harmonic minor its distinctive sound."

- question: "The melodic minor scale uses different notes when ascending versus descending — this is an intentional feature, not an error."
  type: true-false
  answer: true
  explanation: "Melodic minor raises the sixth and seventh scale degrees when ascending (to avoid the awkward augmented second of harmonic minor and to provide a smooth stepwise line to the tonic), then reverts to natural minor when descending (since the leading tone pull is less important when moving away from the root). The asymmetry is deliberate and reflects different melodic needs in each direction."

- question: "Why was the harmonic minor scale developed, and what acoustic problem did it create?"
  type: short-answer
  answer: "Harmonic minor was developed to create a leading tone (raised 7th) with a strong half-step pull toward the tonic, enabling a dominant chord with a major quality. However, raising the 7th creates an augmented second interval between scale degrees 6 and 7, which is difficult to sing and sounds awkward in melodic passages."
  explanation: "In natural minor, the seventh degree is a whole step below the tonic, which weakens the sense of resolution. Raising it by a half step creates the same leading-tone pull that makes major keys feel so conclusive. The tradeoff is an augmented second (three half steps) between b6 and #7, which is melodically awkward. The melodic minor scale was developed to solve this problem in linear passages."
```
