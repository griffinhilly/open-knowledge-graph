...
- 'Dominant seventh' refers to a specific interval formula (major triad + minor seventh), not just any chord with a seventh built on the dominant scale degree.
- The diminished seventh chord uses a diminished seventh interval, which is enharmonically equivalent to a major sixth but has a different harmonic function.

## Questions

```yaml
- question: "What distinguishes a dominant seventh chord (7) from a major seventh chord (Maj7)?"
  type: multiple-choice
  options:
    - "A major triad with a minor seventh vs. a major triad with a major seventh"
    - "A minor triad with a major seventh vs. a major triad with a minor seventh"
    - "Both use major triads; they differ only in the root note"
    - "The dominant seventh uses a diminished fifth; the major seventh uses a perfect fifth"
  answer: 0
...
- question: "A fully diminished seventh chord (°7) is built entirely from stacked minor thirds, meaning every adjacent pair of notes is the same interval apart."
  type: true-false
  answer: true
  explanation: "The fully diminished seventh stacks three minor thirds (3 semitones each): root → minor third → diminished fifth → diminished seventh. This symmetry divides the octave into four equal parts, which is why diminished seventh chords can be enharmonically respelled and reinterpreted in multiple keys."

- question: "Why does the dominant seventh chord (V7) create such strong harmonic tension that resolves naturally to the tonic?"
  type: short-answer
  answer: "It contains a tritone — the dissonant interval between its third and seventh — which resolves by contrary motion: the third rises a half-step to the tonic while the seventh falls a half-step to the third of the tonic chord."
  explanation: "In the key of C major, the G7 chord contains B (the third) and F (the seventh), which form a tritone. B wants to rise to C and F wants to fall to E — the two notes move in opposite directions toward the tonic chord. This contrary resolution is the source of the V7–I cadence's strong sense of arrival."
```
