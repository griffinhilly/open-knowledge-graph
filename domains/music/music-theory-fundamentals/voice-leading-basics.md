...
```yaml
- question: "When moving from a C major chord to an F major chord, which voice motion approach generally produces the smoothest result?"
  type: multiple-choice
  options: ["Move every voice by the largest interval possible", "Keep common tones stationary and move other voices by step", "Always move all voices in the same direction (parallel motion)", "Double the third of the new chord in all voices"]
  answer: 1
