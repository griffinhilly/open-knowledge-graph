...
- You cannot use sharps and flats together in the same scale (e.g., a scale cannot have both F♯ and B♭) — scales use one type consistently.
- Every note name must appear exactly once; writing C-D-E-F♯-G-A-B is not the F♯ major scale (which requires E♯, not F).

## Questions

```yaml
- question: "Which of the following correctly applies the W-W-H-W-W-W-H pattern to build a D major scale?"
  type: multiple-choice
  options: ["D-E-F-G-A-B-C-D", "D-E-F♯-G-A-B-C♯-D", "D-E-F♯-G♯-A-B-C♯-D", "D-E♭-F-G-A-B♭-C-D"]
  answer: 1
  explanation: "Starting on D: W=E, W=F♯ (F is only a half step from E, so we raise it), H=G, W=A, W=B, W=C♯, H=D. Option A omits the required sharps; option C adds an incorrect sharp on G; option D uses flats that belong to a different key."

- question: "A major scale can use both sharps and flats simultaneously — for example, having both F♯ and B♭ in the same scale."
  type: true-false
  answer: false
  explanation: "Major scales use either sharps or flats consistently, never both. Each key signature is built from one type. Mixing accidentals would violate the W-W-H-W-W-W-H interval pattern and produce a set of notes that belongs to no standard major key."

- question: "Why must every letter name (A through G) appear exactly once in a major scale?"
  type: short-answer
  answer: "Each letter name represents a distinct scale degree. Repeating or skipping a letter misrepresents the interval structure and makes the scale impossible to read correctly in standard notation."
  explanation: "The W-W-H-W-W-W-H pattern steps through each pitch class in alphabetical order. Writing F and F♯ instead of F and G♭ would give you two versions of the same letter, breaking the rule that each of the seven letter names appears once. Correct spelling ensures every step reads as a distinct interval and is consistent with the key signature."
```
