...
- Absence of explicit prejudice does not indicate absence of bias; implicit measures routinely detect bias in self-described egalitarians.
- Prejudice reduction at the individual level does not automatically translate to reduced discrimination if institutional structures remain unchanged.

## Questions

```yaml
- question: "A hiring manager genuinely believes they are unbiased but consistently rates candidates from one ethnic group lower during ambiguous, time-pressured interviews. This pattern is best described as:"
  type: multiple-choice
  options: ["Explicit prejudice driving overt discrimination", "Aversive racism expressed in an ambiguous context", "Stereotyping with no affective component", "Institutional discrimination caused by written policy"]
  answer: 1
...
- question: "Explain why the affective, cognitive, and behavioral components of intergroup bias can operate independently of one another."
  type: short-answer
  answer: "Social norms can suppress discriminatory behavior even when prejudiced attitudes and stereotypes remain active (a prejudiced person behaves equitably in public to avoid social sanction). Institutional processes can produce discriminatory outcomes without individual prejudice. And stereotypes can be held with little emotional valence — as descriptive beliefs rather than hostile attitudes. Because different mechanisms control each component, they can diverge."
  explanation: "The tripartite model's key insight is that attitude-behavior consistency is not automatic. Social context, monitoring, and structural factors all modulate which component becomes expressed. This explains why measuring only one component (e.g., explicit attitude surveys) underestimates the full pattern of bias."
```
