...
- Electricity flows out of the battery and gets used up by the bulb. (Electricity flows in a complete loop — charges leave the battery and return to it. The bulb converts electrical energy to light and heat.)
- A single wire from battery to bulb is enough. (You need a complete loop for current to flow — one wire from the battery to the bulb and another wire back.)
- The bulb uses up all the electricity. (The same amount of charge flows out of the battery and back into it; the bulb converts some energy, not the charges themselves.)
- Thicker wires always make a brighter bulb. (Thicker wires have less resistance, which can allow more current, but the battery and bulb also matter.)

## Questions

```yaml
- question: "What must be true for electric current to flow through a circuit?"
  type: multiple-choice
  options: ["The wires must be thick", "The loop must be complete with no gaps", "There must be two batteries", "The bulb must be large"]
  answer: 1
  explanation: "Electric current needs a complete, unbroken loop to flow. Any gap in the circuit stops the flow of electricity."

- question: "A switch works by opening or closing a gap in a circuit."
  type: true-false
  answer: true
  explanation: "When a switch is open, there is a gap in the circuit and current cannot flow. When the switch is closed, the gap is bridged and current flows."

- question: "Name the three main parts of a simple circuit."
  type: short-answer
  answer: "A power source (battery), a conductor (wire), and a load (light bulb or other device)."
  explanation: "Every simple circuit needs something to provide energy, something to carry the current, and something to use the energy."
```
