...
- A single fixed pulley reduces the force needed. (A single fixed pulley only changes the direction of the force; the effort is the same as lifting directly.)
- Pulleys create extra energy. (Pulleys redirect or spread force over more rope, but the total work stays the same.)
- Pulleys are only used on construction sites. (Pulleys are in window blinds, flagpoles, clotheslines, elevators, and exercise machines.)
- More pulleys always make the job easier in every way. (More pulleys reduce force, but you must pull more rope, so the job takes longer.)

## Questions

```yaml
- question: "What does a single fixed pulley change about the force you use?"
  type: multiple-choice
  options: ["It makes the force smaller", "It changes the direction of the force", "It makes the force larger", "It removes gravity"]
  answer: 1
  explanation: "A single fixed pulley lets you pull down to lift something up. It changes direction but does not reduce the amount of force needed."

- question: "Using more pulleys together can reduce the force needed to lift a heavy object."
  type: true-false
  answer: true
  explanation: "A compound pulley system (multiple pulleys) spreads the load over more sections of rope, so each section carries less of the weight."

- question: "Name one place where you might see a pulley being used."
  type: short-answer
  answer: "A flagpole (also acceptable: elevator, construction crane, window blinds, clothesline, well)."
  explanation: "A flagpole uses a pulley at the top so you can pull a rope down at the bottom to raise the flag up."
```
