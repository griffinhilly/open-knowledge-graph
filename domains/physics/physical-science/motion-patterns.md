...
- All moving objects travel at a constant speed. (Many objects speed up or slow down as they move.)
- You can only find patterns in fast-moving things. (Even slow motions like a snail or a growing plant have patterns.)
- Objects that move in patterns will always continue that pattern forever. (Forces like friction and gravity can change the pattern.)
- Patterns in motion are random and cannot be predicted. (Many motion patterns are very predictable once you identify them.)

## Questions

```yaml
- question: "A ball rolls down a hill and its speed at each second is: 2, 4, 6, 8. What pattern do you see?"
  type: multiple-choice
  options: ["The ball is slowing down", "The ball is moving at a constant speed", "The ball is speeding up by the same amount each second", "The ball is moving randomly"]
  answer: 2
  explanation: "The speed increases by 2 each second, which is a pattern of steady speeding up (constant acceleration)."

- question: "An object moving at a steady speed will cover equal distances in equal amounts of time."
  type: true-false
  answer: true
  explanation: "Constant speed means the same distance is covered each second. If a car goes 10 meters every second, it covers 10 meters in the first second, 10 in the second, and so on."

- question: "How can you figure out the motion pattern of a rolling ball?"
  type: short-answer
  answer: "Record the ball's position at regular time intervals and look at how the distance changes from one interval to the next."
  explanation: "By measuring position over time, you can see if the ball covers the same distance each time (constant speed), more distance each time (speeding up), or less distance (slowing down)."
```
