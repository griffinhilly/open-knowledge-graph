...
- We can see objects because our eyes send out light. (We see because light bounces off objects and enters our eyes.)
- Shadows are dark because they are made of something. (Shadows are simply areas where light is blocked — they are the absence of light, not a substance.)
- The Sun is the only natural light source. (Stars, lightning, fireflies, and some deep-sea creatures also produce light.)
- Shadows are always the same size as the object. (Shadow size changes based on the position and distance of the light source.)

## Questions

```yaml
- question: "Why does a shadow form behind an object?"
  type: multiple-choice
  options: ["The object absorbs the shadow", "Light cannot pass through the object so it blocks light on the other side", "The object creates darkness", "Shadows come from the ground"]
  answer: 1
  explanation: "Light travels in straight lines. When an opaque object blocks the light, it creates a dark area behind it where the light cannot reach — that is the shadow."

- question: "Light travels in straight lines."
  type: true-false
  answer: true
  explanation: "Light travels in straight paths called rays. This is why shadows have clear edges and why you cannot see around corners without a mirror."

- question: "What happens to the size of a shadow when you move the light source closer to the object?"
  type: short-answer
  answer: "The shadow gets bigger (larger)."
  explanation: "When the light source is closer to the object, the light rays spread out more behind the object, making the shadow larger."
```
