...
- A screw is completely different from a ramp. (A screw is an inclined plane wrapped in a spiral around a rod.)
- Screws and nails do the same job the same way. (Nails are wedges pushed straight in; screws use threads to pull themselves in and hold much more tightly.)
- The only screws are the kind in a toolbox. (Jar lids, light bulbs, spiral staircases, and car jacks all use the screw principle.)
- Turning a screw takes less total effort than pushing a nail. (You use less force, but you turn over a longer distance — the total work is similar.)

## Questions

```yaml
- question: "A screw is really which other simple machine wrapped around a rod?"
  type: multiple-choice
  options: ["A lever", "A pulley", "An inclined plane", "A wheel and axle"]
  answer: 2
  explanation: "A screw's threads are an inclined plane (ramp) that spirals around a central rod."

- question: "The spiral ridges on a screw are called threads."
  type: true-false
  answer: true
  explanation: "The raised spiral around a screw is called the thread. It is the part that grips material and pulls the screw forward."

- question: "Name one object besides a metal screw that uses the screw principle."
  type: short-answer
  answer: "A jar lid (also acceptable: light bulb, spiral staircase, car jack, bolt, corkscrew)."
  explanation: "A jar lid has threads that grip the jar's rim, using the same spiral principle as a screw to tighten or loosen."
```
