...
title: "Wave Properties: Amplitude, Frequency, and Wavelength"
domain: physics
...
- Waves carry matter from one place to another. (Waves carry energy, not matter. Water molecules in an ocean wave move up and down but do not travel with the wave.)
- Higher frequency means bigger amplitude. (Frequency and amplitude are independent. You can have a high-frequency, low-amplitude wave or a low-frequency, high-amplitude wave.)
- Wavelength is the height of the wave. (Wavelength is the horizontal distance between identical points on consecutive cycles — like crest to crest. The height is the amplitude.)
- All waves have the same speed, so frequency and wavelength are the same thing. (Different waves travel at different speeds, and for any given wave, higher frequency means shorter wavelength.)

## Questions

```yaml
- question: "If you shake a rope faster, what happens to the wavelength of the waves you create?"
  type: multiple-choice
  options: ["Wavelength increases", "Wavelength decreases", "Wavelength stays the same", "Wavelength becomes zero"]
  answer: 1
  explanation: "For a wave on a rope (where speed is roughly constant), increasing the frequency means each wave cycle takes less distance to complete, so the wavelength decreases."

- question: "The amplitude of a wave determines its loudness (for sound) or brightness (for light)."
  type: true-false
  answer: true
  explanation: "Amplitude measures the energy carried by a wave. For sound waves, greater amplitude means louder sound. For light waves, greater amplitude means brighter light."

- question: "What is the difference between frequency and wavelength?"
  type: short-answer
  answer: "Frequency is the number of wave cycles per second (measured in hertz), while wavelength is the physical distance between consecutive identical points on the wave (like crest to crest, measured in meters)."
  explanation: "Frequency describes how often the wave oscillates in time; wavelength describes how long each oscillation is in space. They are inversely related when wave speed is constant."
```
