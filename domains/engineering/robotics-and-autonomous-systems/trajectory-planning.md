...
- question: "A robot must move from configuration A to configuration B. Joint-space trajectory planning connects A and B with a cubic polynomial: θ(t) = θ₀ + (3/t_f²)·(t_f - t)·(θ_f - θ₀)·t - (2/t_f³)·(t_f - t)·t²·(θ_f - θ₀). At the start (t=0) and end (t=t_f), the velocity is zero. Why is this constraint important?"
  ...
  answer: 1
  explanation: "A trajectory with non-zero velocity at the start or end would produce infinite acceleration (jerk) at those points, as the velocity jumps discontinuously from rest to the commanded velocity. This is mechanically harmful and undesirable for control. Zero velocity at start and end ensures smooth acceleration and deceleration, so the controller only needs to command finite, achievable joint accelerations. This is a fundamental requirement of smooth trajectory design."

- question: "A sampling-based motion planner (like RRT) randomly samples configurations in the robot's configuration space and tries to connect them with simple paths. Why is sampling-based planning often used despite being computationally expensive compared to analytical planning?"
  type: multiple-choice
  options:
    - "Because sampling-based planners never fail to find a solution if one exists"
    - "Because analytical solutions are too complex to implement for arbitrary robot geometries and environments"
    - "Because sampling-based planners scale reasonably to high-dimensional configuration spaces and work with arbitrary obstacle geometries without explicit computation of free space"
    - "Because they are guaranteed to find the shortest path"
  answer: 2
  explanation: "Sampling-based planners like RRT and RRT* work by building a tree of feasible configurations and connections. They scale to high-dimensional spaces (6-DOF arms, humanoid robots with 20+ joints) without requiring explicit enumeration of free space. They also handle arbitrary obstacle shapes naturally: you just check whether a configuration collides with the environment using fast geometric tests (e.g., distance queries). In contrast, analytical planning for complex robots or cluttered environments requires explicit computation of the high-dimensional free space, which is computationally intractable. Sampling-based methods sacrifice optimality (they don't find the shortest path) for scalability and generality."

- question: "A trajectory-planning algorithm connects two joint configurations with a straight line in joint space (linear interpolation between θ_start and θ_end). The resulting end-effector path in Cartesian space is:"
  type: multiple-choice
  options:
    - "Always a straight line, regardless of the robot's kinematics"
    - "Generally a curved path in 3D space, depending on the robot's geometry and kinematics"
    - "A helix if the robot has 6 DOF"
    - "Undefined unless the robot is a pure translational mechanism"
  answer: 1
  explanation: "Linear interpolation in joint space does not correspond to linear motion in Cartesian space. The forward kinematics function f(θ) is nonlinear, so a straight line through joint space maps to a curve in Cartesian space. This is actually a feature: the robot reaches the goal, but the path it traces is whatever the kinematics produces. Straight-line end-effector paths require inverse kinematics at each point along the Cartesian line, which is computationally more expensive and can fail if parts of the line pass through singularities or outside the workspace."

- question: "A cubic polynomial trajectory for joint θ: θ(t) = θ₀ + a·t³ + b·t² + c·t, where the boundary conditions are θ(0) = θ₀, θ(T) = θ_f, θ̇(0) = 0, θ̇(T) = 0. How many unknowns (a, b, c, the time T) must be solved for?"
  type: multiple-choice
  options:
    - "3 unknowns (a, b, c); T is given as a constraint"
    - "4 unknowns; the four boundary conditions over-determine the cubic"
    - "2 unknowns; the boundary conditions are redundant"
    - "It depends on the specific joint; some joints may have different requirements"
  answer: 0
  explanation: "A cubic polynomial θ(t) = θ₀ + a·t³ + b·t² + c·t has four coefficients θ₀, a, b, c. Four boundary conditions (θ(0), θ(T), θ̇(0), θ̇(T)) exactly determine a cubic. Given start/end positions and zero velocity at both ends, you can solve for the coefficients. The motion time T is then chosen based on velocity or acceleration limits."

- question: "Explain why a trajectory must have continuous acceleration (C² continuity) for smooth robot motion, even though joint limits only constrain velocity and position."
  type: short-answer
  answer: "Although joint limits directly constrain position θ and velocity θ̇, the robot's actuators can only produce finite torque τ. By Newton's second law, τ = I·θ̈ (moment of inertia times acceleration). A trajectory with discontinuous acceleration (velocity has a kink) requires infinite torque to execute instantaneously. Practically, this causes control oscillations, mechanical chatter, and wear. Smooth acceleration (C² continuity) ensures the required torques remain finite and reasonable. Higher smoothness (C³ jerk limits) is sometimes required for sensitive applications."
