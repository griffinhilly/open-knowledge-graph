...
- A prototype should look like the finished product. (Prototypes are rough, quick, and made from whatever is available. Their job is to test ideas, not to look good.)
- If the prototype does not work, the idea is bad. (A failed prototype teaches you what to fix. Most successful products went through many prototypes that did not work before the final version.)
- You need expensive materials to build a prototype. (Some of the best prototypes are built from cardboard, tape, paper, and other cheap, easy-to-work-with materials.)
- You should only build one prototype. (Engineers often build many prototypes, each testing a different feature or improvement.)
...
- question: "What is the main purpose of building a prototype?"
  ...
  options: ["To create a beautiful final product", "To test whether a design idea actually works", "To show off to classmates", "To use up leftover materials"]
  answer: 1
  explanation: "A prototype exists to test an idea. It converts a design from something imagined into something physical that can be tried, observed, and learned from. Beauty and polish come much later — if at all."

- question: "A prototype that breaks during testing is a failure and means the idea should be abandoned."
  type: true-false
  answer: false
  explanation: "A prototype that breaks teaches you something valuable — where the design is weak. That information is exactly what you need to make the next version better. Most successful inventions went through many broken prototypes. A prototype only fails if you learn nothing from it."

- question: "Why do engineers build prototypes out of cheap, simple materials like cardboard and tape instead of the real materials?"
  type: short-answer
  answer: "Because the goal is to test the idea quickly, not to build the final product. Cheap materials let you build fast, fail fast, and try again without wasting time or money. If the idea works in cardboard, it is worth investing in better materials. If it does not work, you have not wasted much."
  explanation: "Speed and low cost are the key advantages of rough prototypes. An engineer who spends a week building a beautiful prototype from expensive materials and then discovers the design is flawed has wasted a week. An engineer who builds a cardboard version in an hour discovers the same flaw with far less cost."
```
