...
- The strongest bridge is always the best. (The best bridge is the one that is strong enough, affordable, buildable with available materials, and appropriate for the span. Overbuilding is as much an engineering failure as underbuilding.)
- Suspension bridges are always the best because they span the farthest. (Suspension bridges are expensive, complex, and susceptible to wind. For a short span, a simple beam or arch bridge is far more practical.)
- A bridge fails only if it collapses. (A bridge can fail by bending too much, vibrating dangerously, cracking under repeated loads, or becoming unsafe even if it does not fall.)

## Questions

```yaml
- question: "Which type of bridge uses triangular frames to create a strong, lightweight structure?"
...
- question: "Why do engineers test bridge designs with gradually increasing weight rather than just placing the maximum expected load on the bridge?"
  type: short-answer
  answer: "Gradual loading shows how the bridge behaves at every level of stress. Engineers can see where it starts to bend, flex, or crack before it fails completely. This reveals weak points that can be reinforced. Loading the maximum weight all at once might cause a sudden collapse that teaches nothing about where or why the bridge failed."
  explanation: "Incremental testing is an important engineering practice. By watching a bridge bend slightly at 10 pounds, creak at 20 pounds, and show visible stress at 30 pounds, an engineer learns exactly which parts need strengthening — even if the bridge does not break until 50 pounds. A single pass/fail test misses all that valuable information."
```
