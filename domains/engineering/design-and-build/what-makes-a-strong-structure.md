...
title: What Makes a Strong Structure
domain: engineering
...
- Bigger structures are always stronger. (A poorly designed big structure is weaker than a well-designed small one. Size without good engineering is just more weight to support.)
- Strength comes only from the material. (Shape matters enormously. A flat piece of cardboard cannot hold a book, but the same piece folded into a tube can support several books.)
- If a structure does not fall down, it is strong enough. (A structure might not collapse but could bend, sway, or deform in ways that make it unsafe or unusable. Engineers design for specific performance, not just "doesn't fall.")

## Questions

```yaml
- question: "A flat sheet of paper cannot hold a textbook, but the same paper rolled into a tube can. Why?"
  type: multiple-choice
  options: ["The tube is made of stronger paper", "Rolling the paper adds more material", "The tube shape distributes the force across a larger area and resists bending", "The tube is taller so it balances better"]
  answer: 2
  explanation: "The material is identical — same paper, same weight, same thickness. The only difference is shape. A flat sheet bends easily because force is concentrated along a line. A tube distributes force around its entire circumference, and the circular shape resists collapsing in any direction. Shape is one of the most powerful tools an engineer has."

- question: "If a building does not fall down, that means it is well-engineered."
  type: true-false
  answer: false
  explanation: "Not falling down is the minimum — but a building can also sway too much in wind, develop cracks, vibrate uncomfortably, or deform under load in ways that are unsafe. Engineers design structures to meet specific performance standards, not just to avoid total collapse."

- question: "Name the three main things that determine how strong a structure is."
  type: short-answer
  answer: "Shape, materials, and connections. The shape of the structure determines how forces are distributed. The materials determine how much force each part can handle. The connections determine whether the parts stay together under load."
  explanation: "Engineers control all three factors when designing structures. A bridge might use triangular shapes (strong shape), steel beams (strong material), and riveted joints (strong connections). Changing any one of these factors can dramatically change the structure's strength."
```
