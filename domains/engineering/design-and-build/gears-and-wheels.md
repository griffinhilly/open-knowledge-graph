...
- Gears always spin in the same direction. (Two meshing gears spin in opposite directions. To get both spinning the same way, you need an odd number of gears or a chain/belt connecting them.)
- Bigger gears always spin faster. (Bigger gears spin slower when driven by smaller gears. Their advantage is more force, not more speed.)
- Gears are only found in factories and machines. (Gears are in everyday objects: bicycles, clocks, can openers, hand mixers, wind-up toys.)

## Questions

```yaml
- question: "A large gear with 30 teeth is turning a small gear with 10 teeth. How does the small gear behave?"
  type: multiple-choice
  options: ["It spins at the same speed with the same force", "It spins 3 times faster but with less force", "It spins 3 times slower but with more force", "It does not move because the gears are different sizes"]
  answer: 1
...
- question: "Why do bicycles have different-sized gears that riders can switch between?"
  type: short-answer
  answer: "Different gear sizes let riders trade speed for power. A small gear driving a large wheel is hard to pedal but moves fast — good for flat roads. A large gear driving a small wheel is easy to pedal but moves slowly — good for climbing hills."
  explanation: "Bicycle gear shifting is one of the clearest everyday examples of the gear ratio trade-off. Low gear (easy pedaling, slow speed) uses a large front gear with a large rear gear to reduce the force needed. High gear (hard pedaling, high speed) uses combinations that make the rear wheel turn many times per pedal turn."
```
