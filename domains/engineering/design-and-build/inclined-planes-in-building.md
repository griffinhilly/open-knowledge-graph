...
title: Inclined Planes in Building
domain: engineering
...
- A steeper ramp is always better because it is shorter. (A steeper ramp requires more force. Engineers choose the angle based on how much force is available and how much space they have.)
- Wedges and ramps are completely different things. (A wedge is two inclined planes put together at an angle. An axe blade, a doorstop, and a chisel are all wedges — inclined planes used to split or hold.)
- Screws have nothing to do with ramps. (A screw is an inclined plane wrapped in a spiral around a cylinder. The thread is the ramp. That is why screws go in slowly but hold with tremendous force.)

## Questions

```yaml
- question: "A warehouse needs to move heavy boxes from the ground to a truck bed 4 feet high. Which ramp design requires the least pushing force?"
  type: multiple-choice
  options: ["A 4-foot ramp going straight up (a ladder)", "A 6-foot ramp at a steep angle", "A 16-foot ramp at a gentle angle", "All ramps require the same force regardless of length"]
  answer: 2
  explanation: "The longer and gentler the ramp, the less force is needed to push the load up — but the load has to travel a greater distance. A 16-foot ramp to reach a 4-foot height has a very gentle slope. The 4-foot 'ramp' (straight up) is essentially lifting, providing no mechanical advantage. This is the fundamental inclined plane trade-off: less force requires more distance."

- question: "A screw is an inclined plane wrapped around a cylinder."
  type: true-false
  answer: true
  explanation: "If you cut out a right triangle from paper and wrap it around a pencil, the hypotenuse (the slanted edge) traces a spiral — exactly like a screw thread. Each turn of the screw moves it forward a small distance (the thread spacing) with a large force, just as walking up a gentle ramp lifts you a small height with less effort than climbing straight up."

- question: "Why do mountain roads use switchbacks (zigzag paths) instead of going straight up the mountain?"
  type: short-answer
  answer: "Switchbacks make the road longer but less steep, reducing the force needed for vehicles to climb. A straight road up a steep mountain would require too much engine power for most vehicles. Switchbacks are inclined planes that trade distance for a gentler slope."
  explanation: "This is the inclined plane principle applied to civil engineering. A 1-mile mountain height gained over 20 miles of switchback road has a gentle grade that any car can handle. A 1-mile height gained over 2 miles of straight road would be impossibly steep. The same total elevation is gained, but the force required per foot of travel is much smaller."
```
