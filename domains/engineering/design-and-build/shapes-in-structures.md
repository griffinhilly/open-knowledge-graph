---
id: shapes-in-structures
title: "Shapes in Structures: Triangles and Arches"
domain: engineering
...
- Triangles are strong because the three sides share the weight equally. (The strength comes from rigidity — a triangle's shape is fixed by its three side lengths. It cannot deform without a side changing length, which requires breaking.)
- Arches are strong because they are curved. (The curvature matters, but the strength comes from how the arch converts downward force into compression along its curve. A random curve is not necessarily strong — the shape must direct forces into the supports.)
- You only see triangles in bridges and skyscrapers. (Triangles are everywhere in structures: roof trusses, bicycle frames, crane arms, camera tripods, and folding chairs all use triangles for rigidity.)

## Questions

```yaml
- question: "Why is a triangle stronger than a square in a structure?"
  type: multiple-choice
  options: ["Because triangles have fewer sides to break", "Because a triangle cannot change shape without breaking a side, while a square can be pushed into a parallelogram", "Because triangles are heavier than squares", "Because triangles use more material"]
  answer: 1
  explanation: "A triangle is defined by its three side lengths — there is only one possible shape for those three lengths. Push on a corner, and the sides would have to stretch, shrink, or break for the shape to change. A square has four sides, and those same four sides can form an infinite number of parallelograms without any side changing length. That is why a square frame wobbles and a triangular frame holds firm."

- question: "Adding a diagonal brace to a square frame makes it stronger because it divides the square into two triangles."
  type: true-false
  answer: true
  explanation: "The diagonal brace converts the deformable square into two rigid triangles. The square can no longer collapse into a parallelogram because the diagonal prevents the corners from shifting. This technique — called triangulation — is used in construction, furniture, and engineering structures everywhere."

- question: "How does an arch support heavy loads?"
  type: short-answer
  answer: "An arch converts downward force into compression along its curved shape, pushing the force outward and down into the supports at each end. This spreads the load across the entire arch instead of concentrating it at one point in the middle."
  explanation: "In a flat beam, the center bears the most stress and is where failure usually starts. An arch redirects that central force along its curve so every part of the arch shares the load. Ancient Romans built aqueducts and buildings with arches that have stood for 2,000 years — the shape is that effective at distributing force."
```
