...
- Measuring does not need to be exact for building projects. (Even small measurement errors accumulate. If four pieces are each a quarter-inch too long, the final structure is a full inch off — which might mean parts do not fit together.)
- You should start measuring from the very end of the ruler. (Many rulers have a small gap before the zero mark. Always check where zero is and start from there.)
- Cutting is the hard part; measuring is easy. (Accurate measurement is actually the harder skill. A cut is only as good as the measurement and marking that guided it.)

## Questions

```yaml
- question: "Why do carpenters say 'measure twice, cut once'?"
  type: multiple-choice
  options: ["Because measuring is fun and they want to do it twice", "Because cutting a piece too short cannot be fixed — you have to start over with new material, so double-checking the measurement prevents waste", "Because rulers are unreliable and need two readings to be accurate", "Because cutting twice is dangerous"]
  answer: 1
...
- question: "Why is it important to mark your measurement on the material before cutting?"
  type: short-answer
  answer: "A mark gives you a visible line to follow while cutting, which is much more accurate than trying to hold a ruler against the material while cutting at the same time. Without a clear mark, the cut is likely to wander off the intended line."
  explanation: "Marking separates the measurement step from the cutting step, which lets you focus on each one individually. Trying to measure and cut simultaneously requires you to hold the ruler, hold the material, and operate the cutting tool all at once — which is both inaccurate and unsafe."
```
