...
- Physicalism does not entail that psychology reduces to neuroscience; non-reductive physicalism holds that mental properties are real but not definable in purely physical terms.
- Affirming physicalism does not resolve the hard problem; it merely relocates the explanatory burden.

## Questions

```yaml
- question: "Which version of physicalism holds that every individual mental event is identical to some physical brain event, but mental types (e.g., 'pain') need not correspond to a single physical type?"
  type: multiple-choice
  options: ["Type identity theory", "Token physicalism", "Eliminative materialism", "Supervenience physicalism"]
  answer: 1
...
- question: "What is the multiple realizability objection to type identity theory, and which version of physicalism was developed partly in response to it?"
  type: short-answer
  answer: "Multiple realizability: pain (or any mental state) can be physically realized in very different ways across species and systems, so 'pain' cannot be identical to a single physical type like C-fiber firing. Token physicalism and functionalism were developed in response, allowing mental types to be multiply realized while preserving a physicalist commitment."
  explanation: "Type identity theory requires each mental type to correspond to exactly one physical type. But if an octopus feels pain through a completely different neural mechanism than a human, and a hypothetical silicon robot could also feel pain, then 'pain' cannot be identical to any single physical property. This argument, associated with Hilary Putnam, pushed physicalists toward weaker identity claims or functional characterizations of mental states."
```
