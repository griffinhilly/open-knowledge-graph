...
- Confusing opinions with premises (opinions can be premises, but assertions can be true). - Thinking arguments must be formal or long (simple statements like 'It's raining, so bring an umbrella' are arguments). - Assuming conclusions always come last (they can appear anywhere).

## Questions

```yaml
- question: "Which of the following is an example of an argument?"
  type: multiple-choice
  options: ["The sky is blue.", "I think cats are better than dogs.", "It's raining, so you should bring an umbrella.", "What time is it?"]
  answer: 2
...
- question: "What is the difference between a premise and a conclusion in an argument?"
  type: short-answer
  answer: "A premise is a statement offered as a reason or evidence; a conclusion is the statement the premises are meant to establish or support."
  explanation: "The logical relationship is asymmetric: premises do the supporting, the conclusion is what gets supported. Identifying which role each statement plays is the first step in evaluating whether the reasoning actually works."
```
