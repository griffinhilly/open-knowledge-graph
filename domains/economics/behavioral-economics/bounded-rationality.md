...
```yaml
- question: "Simon's concept of satisficing means that decision-makers..."
  type: multiple-choice
  options:
    - "Always choose the worst available option due to cognitive limitations"
    - "Search through options sequentially and accept the first one that meets a minimum acceptable threshold"
    - "Randomly select among available alternatives"
    - "Maximize utility but with a small error term"
  answer: 1
  explanation: "Satisficing combines 'satisfy' and 'suffice.' Rather than exhaustively evaluating all options to find the optimum, a satisficing agent searches sequentially and stops when an alternative meets their aspiration level. This is adaptive given real-world constraints: the cost of continued search (time, cognitive effort) may exceed the expected improvement from finding a marginally better option. Satisficing is not random or lazy — it is a cognitively efficient strategy that often produces good-enough outcomes."

- question: "Bounded rationality implies that people are irrational and their decisions should be overridden by expert optimization."
  type: true-false
  answer: false
  explanation: "Simon was careful to distinguish bounded rationality from irrationality. Boundedly rational agents are doing the best they can given realistic constraints — limited information, limited computation, limited time. Their strategies (satisficing, using heuristics) are often well-adapted to the environments they face. The ecological rationality program (Gigerenzer) goes further, arguing that simple heuristics can outperform complex optimization in uncertain, real-world environments. Bounded rationality does not mean people are stupid; it means the standard model's assumptions about human cognition are unrealistic."

- question: "How does bounded rationality challenge the standard economic assumption of utility maximization?"
  type: short-answer
  ...
  explanation: "The challenge is not merely that people make occasional errors around a rational baseline — it is that the decision process itself differs qualitatively from optimization. People use heuristics, are influenced by framing, have inconsistent time preferences, and are sensitive to contextual factors that should be irrelevant under standard theory. Bounded rationality opened the door for prospect theory, heuristics and biases research, and the entire behavioral economics enterprise."
```
