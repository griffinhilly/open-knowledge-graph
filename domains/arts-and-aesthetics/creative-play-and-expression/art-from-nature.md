---
id: art-from-nature
title: Art from Nature
domain: arts-and-aesthetics
course: creative-play-and-expression
prerequisites:
- id: texture-and-touch-in-art
  type: soft
- id: looking-at-colors-in-nature
  type: hard
builds-toward: []
tags:
- nature
- found-materials
- outdoor-art
stage: pre-formal
status: draft
---

# Art from Nature

## Core Idea
You can make amazing art using things you find in nature! Leaves, sticks, stones, flowers, pinecones, and seeds can all become part of your artwork. Nature gives you free art materials in beautiful colors, shapes, and textures.

## How It's Best Learned
Take nature walks to collect materials (leaves, sticks, small stones, flowers, bark, seeds). Arrange nature items into pictures or patterns on the ground or paper. Glue nature items onto cardboard for a nature collage. Make leaf prints by painting a leaf and pressing it on paper. Build small sculptures with sticks and stones.

## Common Misconceptions
- Art must be made with store-bought supplies; nature provides endless free art materials.
- Nature art has to last forever; temporary art made outside is beautiful even if the wind blows it away.
- Leaves and sticks are just trash; they are beautiful materials with unique shapes, colors, and textures.

## Questions

```yaml
- question: "Which of these nature items could you use to make art?"
  type: multiple-choice
  options: ["A plastic toy", "A leaf", "A cell phone", "A sock"]
  answer: 1
  explanation: "A leaf is from nature and makes great art! You can glue it, paint it, press it, or arrange it into a design."

- question: "What can you do with a stick to make art?"
  type: multiple-choice
  options: ["Nothing, sticks are not for art", "Use it to draw in sand or arrange it in a sculpture", "Only throw it", "Only use it as a wand"]
  answer: 1
  explanation: "Sticks are wonderful art materials! You can draw in sand, arrange sticks into shapes, or build little sculptures with them."

- question: "You can find art materials outside in nature."
  type: true-false
  answer: true
  explanation: "Yes! Leaves, sticks, stones, flowers, pinecones, and seeds are all art materials that nature gives you for free."

- question: "Nature art is not real art because it does not last forever."
  type: true-false
  answer: false
  explanation: "Nature art is absolutely real art! Some of the most beautiful art in the world is made outdoors with nature materials. It does not need to last forever to be special."

- question: "Collect some nature items and arrange them into a picture or pattern. What did you make?"
  type: short-answer
  answer: "Example: 'I used yellow leaves for a sun, green leaves for grass, and sticks for a fence. I made a nature picture of a garden.'"
  explanation: "A good answer describes what nature items were used and how they were arranged into something creative."
```

## Explainer

**Nature is full of beautiful art materials** just waiting for you to find them. Every leaf, stick, stone, flower, and pinecone has its own shape, color, and texture. When you pick them up and arrange them, you become a nature artist!

**You can make many kinds of art with nature items.** Arrange leaves and flowers into a picture on the ground. Glue sticks and pebbles onto cardboard. Press a painted leaf onto paper to make a leaf print. Stack stones into a little tower. Line up pinecones in a row. Nature art can be flat or three-dimensional, big or small, colorful or simple.

**One wonderful thing about nature art is that it can be temporary.** You might arrange a beautiful spiral of stones and leaves on the ground, enjoy it, and then leave it for others to find. The wind might scatter it, and that is okay. This kind of art is about the experience of making it, not about keeping it forever. Some artists make temporary art outdoors on purpose, and they take photos to remember it.

**Next time you are outside, look down.** You might find the perfect leaf, an interesting stick, or a smooth stone. Pick it up and think about what you could make with it. Nature is the world's oldest and most generous art supply store, and everything in it is free!
