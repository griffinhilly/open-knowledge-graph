---
id: making-faces-and-expressions-in-art
title: Making Faces and Expressions in Art
domain: arts-and-aesthetics
course: creative-play-and-expression
prerequisites:
- id: drawing-and-scribbling
  type: hard
builds-toward:
- self-portraits
- puppets-and-storytelling-through-art
- body-painting-and-face-painting
tags:
- faces
- expressions
- emotions
stage: pre-formal
status: draft
---

# Making Faces and Expressions in Art

## Core Idea
Faces show feelings! A smile means happy, a frown means sad, and wide eyes mean surprised. You can draw, paint, or build faces that show different feelings. Making faces in art helps you think about emotions and how people feel.

## How It's Best Learned
Look in a mirror and make different faces (happy, sad, angry, surprised, silly). Draw those faces on paper. Use paper plates to make face masks. Build faces from collage materials. Read picture books and talk about how characters' faces show their feelings. Play an emotion guessing game with drawn faces.

## Common Misconceptions
- Faces must have realistic proportions to be good; simple circle faces with dots for eyes are wonderful art.
- You can only draw happy faces; art is a safe place to express all feelings, including sad, angry, and scared.
- Drawing faces is too hard for young children; even very young children can make expressive faces with simple marks.

## Questions

```yaml
- question: "If you want to draw a happy face, what shape would the mouth be?"
  type: multiple-choice
  options: ["A straight line", "A curved line going up like a smile", "A curved line going down like a frown", "No mouth at all"]
  answer: 1
  explanation: "A happy face has a mouth that curves up into a smile! Just a simple curved line can show happiness."

- question: "What does a face with a frown and droopy eyes show?"
  type: multiple-choice
  options: ["Happiness", "Excitement", "Sadness", "Surprise"]
  answer: 2
  explanation: "A frown and droopy eyes usually show sadness. The way you draw a face tells people what that face is feeling."

- question: "You can show different feelings by changing the shape of the mouth and eyes in your drawing."
  type: true-false
  answer: true
  explanation: "Yes! A smile shows happy, a frown shows sad, wide open eyes show surprise, and squinty eyes with a frown show anger. Small changes make big differences."

- question: "Art should only show happy faces, never sad or angry ones."
  type: true-false
  answer: false
  explanation: "Art can show all feelings! It is healthy to draw sad, angry, or scared faces. Art is a safe way to explore and express every kind of emotion."

- question: "Draw two faces showing two different feelings. What feelings did you draw?"
  type: short-answer
  answer: "Example: 'I drew a happy face with a big smile and a surprised face with a round open mouth and big eyes.'"
  explanation: "A good answer names at least two different feelings and describes how the face shows each one through features like the mouth, eyes, or eyebrows."
```

## Explainer

**Faces are one of the most important things humans look at.** Even as a baby, you learned to look at faces to understand how people feel. A smile means someone is happy. A frown means someone is sad. Raised eyebrows mean someone is surprised. We read faces every day!

**You can make faces in your art to show feelings.** Start with a circle for the head. Add two dots for eyes. Then draw a mouth. If the mouth curves up, the face looks happy. If it curves down, the face looks sad. If the mouth is a big open circle, the face looks surprised. Just those tiny changes make the face show a completely different feeling!

**Eyes and eyebrows help show feelings too.** Draw eyebrows that go up in the middle for a worried face. Draw eyebrows that go down in the middle for an angry face. Make the eyes wide open for surprise or half-closed for sleepy. You can even add tears for crying or rosy circles on the cheeks for blushing.

**Making faces in art helps you understand feelings, yours and other people's.** When you draw an angry face, you think about what anger looks like. When you draw a scared face, you think about what fear looks like. This helps you notice and name feelings in real life too. Art is a wonderful way to explore the big world of emotions, and every feeling is welcome in your art!
