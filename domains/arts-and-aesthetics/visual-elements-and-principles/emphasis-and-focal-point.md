...
title: Emphasis and Focal Point
...
- A composition must have only one focal point — a hierarchy with a primary, secondary, and tertiary focal point creates a richer viewing experience.
- Putting the subject in the center automatically makes it the focal point — without contrast, a centered subject can still be visually weak.

## Questions

```yaml
- question: "An artist wants to create a strong focal point in a landscape painting. Which technique would be MOST effective if no compositional rearrangement is allowed?"
  type: multiple-choice
  options: ["Move the subject to the center of the canvas", "Increase contrast in value around the subject while softening edges elsewhere", "Add more detail evenly across the whole painting", "Use only warm colors throughout the composition"]
  answer: 1
...
- question: "Why does centering the subject of a composition not guarantee it becomes the focal point?"
  type: short-answer
  answer: "Placement is only one of several factors creating emphasis. A centered subject with low contrast, soft edges, and similar detail to its surroundings will recede visually despite its position."
  explanation: "Emphasis is produced by differential — contrast in value, edge sharpness, color saturation, or detail — between the focal area and its surroundings. Center placement exploits natural eye-tracking tendencies, but those tendencies are overridden by stronger contrast cues elsewhere. A dull gray circle centered on a dull gray background is invisible; a bright orange dot off-center on gray commands the eye."
```
