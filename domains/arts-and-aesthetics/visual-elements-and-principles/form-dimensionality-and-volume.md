...
title: 'Form: Dimensionality and Volume'
...
- Thinking 3D thinking only matters for realistic representation—understanding form and dimensionality improves all design, even abstract work.
- Confusing form with shape—shape is 2D (outline), form is 3D (with volume and depth).

## Questions

```yaml
- question: "An artist draws a circle. What would transform that 2D shape into the illusion of a 3D form?"
  type: multiple-choice
  options: ["Outlining it with a thicker line", "Adding shading that suggests a light source hitting one side", "Moving it to a different position on the canvas", "Changing its color to a warmer hue"]
  answer: 1
