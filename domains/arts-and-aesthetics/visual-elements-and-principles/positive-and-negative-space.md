...
- Negative space is wasted space that should be filled — it is a deliberate design choice; crowded compositions often suffer from ignoring it.
- Negative space is always the background color — in abstract work, figure and ground can reverse (Rubin's vase illusion).

## Questions

```yaml
- question: "A graphic designer creates a logo where the white space between two shapes forms the outline of an arrow. This is an example of:"
  type: multiple-choice
  options: ["Additive color mixing", "Negative space used as an active compositional element", "A contour line technique", "Symmetrical balance"]
  answer: 1
  explanation: "The arrow is formed entirely by the negative (background) space between positive shapes — a classic example of negative space carrying meaning rather than acting as empty filler. FedEx's logo uses this technique with a hidden arrow between the E and x."

- question: "Leaving large areas of empty space around a subject weakens a composition because it wastes potential space that could contain more visual information."
  type: true-false
  answer: false
  explanation: "This is one of the most common misconceptions in design and art. Negative space is a deliberate compositional tool: it creates breathing room, directs attention toward the subject, and can itself carry meaning. Overcrowding a composition by filling all available space typically reduces clarity and impact."

- question: "What is the figure-ground relationship, and why does it matter in composition?"
  type: short-answer
  answer: "The figure-ground relationship is the perceptual distinction between the main subject (figure) and the area surrounding it (ground). It matters because the eye reads figure and ground together — how much ground there is, its shape, and its contrast with the figure all affect balance, emphasis, and readability."
  explanation: "Neither figure nor ground exists in isolation; each defines the other. A large, active ground shape can compete with or complement the figure. In ambiguous images like Rubin's vase, the figure and ground can flip, which is only possible because the ground has been given a shape with its own visual weight."
```
