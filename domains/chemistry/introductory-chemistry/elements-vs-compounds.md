...
- A compound is not the same as a mixture — in a compound, atoms are chemically bonded in fixed proportions, while in a mixture, substances are physically combined in any proportion.
- The properties of a compound are usually very different from the properties of the individual elements that compose it (sodium is a reactive metal and chlorine is a toxic gas, but sodium chloride is table salt).
- Not all molecules are compounds — a molecule of oxygen gas (O2) contains only one type of atom, so it is an element, not a compound.
...
- question: "Which of the following is a compound?"
  ...
  options: ["Oxygen gas (O2)", "Gold (Au)", "Carbon dioxide (CO2)", "Helium (He)"]
  answer: 2
  explanation: "Carbon dioxide (CO2) is a compound because it contains two different elements — carbon and oxygen — chemically bonded together. Oxygen gas is a molecule but contains only one element. Gold and helium are single elements."

- question: "A compound can be separated into its elements by filtering."
  type: true-false
  answer: false
  explanation: "Filtering is a physical separation method that works for mixtures but not compounds. Because the atoms in a compound are held together by chemical bonds, only a chemical change (like electrolysis or a chemical reaction) can break a compound into its component elements."

- question: "Explain why water (H2O) is classified as a compound rather than an element."
  type: short-answer
  answer: "Water contains two different elements — hydrogen and oxygen — chemically bonded together in a fixed ratio, which is the definition of a compound."
  explanation: "A compound must contain atoms of more than one element joined by chemical bonds. Each water molecule has two hydrogen atoms bonded to one oxygen atom (a fixed 2:1 ratio). Because two different elements are involved and they are chemically combined, water is a compound, not an element."
```
