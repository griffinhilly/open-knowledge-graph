---
id: single-replacement-reactions
...
- Not any element can replace any other — the replacing element must be more reactive than the one being replaced.
- The displaced element does not disappear — it becomes a free element (often depositing as a solid or forming a gas).
- Single replacement is different from double replacement — in single replacement, one element swaps; in double replacement, two compounds swap partners.
- Reactivity is an inherent property of each element, not something that changes based on the situation.

## Questions

```yaml
- question: "In the reaction Zn + CuSO4 → ZnSO4 + Cu, what is being replaced?"
  type: multiple-choice
  options: ["Zinc is replaced by copper", "Copper is replaced by zinc", "Sulfur is replaced by zinc", "Oxygen is replaced by copper"]
  answer: 1
...
- question: "What determines whether a single replacement reaction will occur between a metal and a compound containing a different metal?"
  type: short-answer
  answer: "The relative reactivity of the two metals — the free metal must be more reactive than the metal in the compound."
  explanation: "Elements have an inherent tendency to react, called reactivity. An activity series ranks metals from most to least reactive. A metal higher on the series can replace a metal lower on the series from a compound, but not the other way around. This is why zinc can replace copper but copper cannot replace zinc."
```
