...
title: "Density: How Tightly Packed Is It?"
domain: chemistry
...
- id: solids-liquids-gases
  type: soft
builds-toward:
- states-of-matter-properties-and-transitions
tags:
- density
- mass
- volume
...
- Thinking density means the same as weight (a heavy object is not always denser — a huge block of foam is heavy but not dense).
- Believing big objects are always denser than small objects.
- Assuming all metals have the same density (aluminum is much less dense than lead).
- Thinking sinking and floating depend only on weight (it depends on density relative to the liquid).

## Questions

```yaml
- question: "A wooden block and a steel block are the same size. The steel block is much heavier. What does this tell you about their densities?"
  type: multiple-choice
  options:
    - "They have the same density because they are the same size"
    - "The wooden block is denser because wood is a natural material"
    - "The steel block is denser — it has more mass packed into the same volume"
    - "Density cannot be compared without knowing the color of each block"
  answer: 2
...
- question: "Why does a steel ship float even though steel sinks in water?"
  type: short-answer
  answer: "A steel ship is hollow and filled with air, which gives it a very large volume compared to its mass. The overall density of the ship (steel plus the air inside) is less than the density of water, so it floats. A solid chunk of steel would sink because it is denser than water."
  explanation: "Density is mass divided by TOTAL volume. A ship is mostly air inside its steel shell. Even though the steel itself is dense, the enormous volume of air brings the average density of the whole ship below water's density. If the ship fills with water (loses that air space), it sinks — as the Titanic demonstrated."
```
