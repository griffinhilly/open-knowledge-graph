...
- Thinking any change in appearance means a chemical change (cutting, crushing, and coloring are physical changes).
- Believing physical changes are always easy to reverse (smashing a glass is physical but hard to undo).
- Confusing dissolving (usually physical) with a chemical reaction.
- Thinking the words "physical" and "chemical" refer to how dramatic the change looks.

## Questions

```yaml
- question: "You cut a piece of aluminum foil into small pieces. Is this a physical or chemical change?"
  type: multiple-choice
  options:
    - "Chemical change — the foil changed shape, so a new substance must have formed"
    - "Physical change — the foil is still aluminum, just in smaller pieces"
    - "Both — cutting is physical and chemical at the same time"
    - "Neither — nothing changed because it is still shiny and silver"
  answer: 1
...
- question: "How can you tell whether a change is physical or chemical?"
  type: short-answer
  answer: "Ask whether the same substance is still there. In a physical change, the material keeps its identity (ice is still water, cut paper is still paper). In a chemical change, new substances form with different properties (wood becomes ash, iron becomes rust). Signs like gas production, color change, temperature change, or new smells suggest a chemical change."
  explanation: "The fundamental test is identity, not appearance. Physical changes alter form — size, shape, state — without changing what the material is. Chemical changes create entirely new materials. The signs of chemical change are clues, but the real question is always: is this still the same stuff?"
```
