...
- Thinking all solids are hard — cotton balls, sponges, and clay are soft solids.
- Believing that if you can change a solid's shape (like bending a paper clip), it becomes a liquid.
- Assuming heavier objects are always bigger or that bigger objects are always heavier.
- Confusing the shape of a solid with the shape of its container (solids keep their own shape).

## Questions

```yaml
- question: "Which property makes a diamond useful for cutting glass?"
  type: multiple-choice
  options:
    - "Its color — diamonds are clear like glass"
    - "Its hardness — diamond is harder than glass and can scratch it"
    - "Its size — diamonds are small enough to fit on a cutting tool"
    - "Its weight — diamonds are heavy enough to press through glass"
  answer: 1
  explanation: "Diamond is the hardest natural material. Because it is harder than glass, it can scratch and cut glass. Color, size, and weight are real properties of diamonds, but they are not the reason diamonds can cut glass."

- question: "A rubber band is not a solid because you can stretch it into a different shape."
  type: true-false
  answer: false
  explanation: "False. A rubber band is absolutely a solid. Solids can be flexible — they bend, twist, or stretch — and still be solids. The key test is that a rubber band does not flow to fill a container the way a liquid would. It keeps a definite shape on its own."

- question: "Name two properties you could use to describe a wooden block."
  type: short-answer
  answer: "Possible answers include: hard, rough or smooth texture, rectangular shape, brown color, lightweight, rigid (does not bend easily), opaque (you cannot see through it)."
  explanation: "Solids have many observable properties. For a wooden block, you might notice its hardness (it does not dent easily), its texture (smooth if sanded, rough if unfinished), its color, and its rigidity. Any two accurate, observable properties count."
```
