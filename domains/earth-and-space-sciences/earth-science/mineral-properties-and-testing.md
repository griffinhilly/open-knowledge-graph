...
title: Mineral Properties and Testing
...
- Color is the best way to identify a mineral. Many different minerals share the same color, and some minerals come in multiple colors (quartz can be clear, pink, purple, white, or smoky).
- Hardness means the same as strength or toughness. A diamond is the hardest mineral but can be shattered with a hammer — hardness is about scratch resistance, not impact resistance.
- Shiny minerals are always metallic. Glassy luster (like quartz) is not the same as metallic luster (like pyrite).

## Questions

```yaml
- question: "A student finds a golden-colored mineral and thinks it might be gold. Which property test would be MOST helpful in telling real gold from fool's gold (pyrite)?"
  type: multiple-choice
  options:
    - "Color — real gold is more yellow"
    - "Streak — gold leaves a golden streak while pyrite leaves a greenish-black streak"
    - "Size — real gold nuggets are always bigger"
    - "Weight alone — just hold them in your hand"
  answer: 1
  explanation: "Streak is one of the most diagnostic tests. When rubbed on a streak plate, gold leaves a golden-yellow streak, while pyrite (fool's gold) leaves a greenish-black streak. Color alone is unreliable because both look golden on the surface."

- question: "On the Mohs hardness scale, a mineral with a hardness of 7 can scratch a mineral with a hardness of 5, but a mineral of 5 cannot scratch a mineral of 7."
  type: true-false
  answer: true
  explanation: "The Mohs scale ranks scratch resistance. A harder mineral always scratches a softer one, never the reverse. Quartz (7) will scratch apatite (5), but apatite cannot scratch quartz."

- question: "What is the difference between a mineral's cleavage and fracture?"
  type: short-answer
  answer: "Cleavage is when a mineral breaks along smooth, flat planes because of its crystal structure (like mica splitting into thin sheets). Fracture is when a mineral breaks along rough, irregular surfaces with no flat planes (like quartz shattering into curved, shell-like pieces)."
  explanation: "Whether a mineral shows cleavage or fracture depends on how its atoms are bonded. If there are weak planes in the crystal structure, the mineral cleaves along them. If bonds are equally strong in all directions, the mineral fractures irregularly."
```
