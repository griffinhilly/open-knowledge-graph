---
id: severe-weather-storms-and-tornadoes
title: Severe Weather (Storms and Tornadoes)
domain: earth-and-space-sciences
course: earth-and-weather
prerequisites:
- id: what-is-weather
  type: hard
- id: rainy-days
  type: hard
- id: what-is-wind
  type: hard
builds-toward:
- weather-forecasting-basics
- severe-weather-systems
tags:
- weather
- storms
- tornadoes
- thunderstorms
- safety
stage: concrete-operations
status: validated
---

# Severe Weather (Storms and Tornadoes)

## Core Idea
Sometimes weather becomes dangerous. Thunderstorms bring heavy rain, lightning, and strong winds. Tornadoes are spinning columns of air that reach from storm clouds to the ground and can cause great damage. Hurricanes are huge storms that form over warm ocean water. Knowing about severe weather helps people stay safe by taking shelter and listening to weather warnings.

## How It's Best Learned
Use age-appropriate videos showing thunderstorms, tornadoes, and hurricanes from a safe observational perspective. Focus heavily on safety -- what to do during each type of severe weather. Practice a severe weather drill. Discuss warning signs (dark skies, sudden wind shifts, hail) without creating fear.

## Common Misconceptions
- Lightning cannot hurt you if you are inside. Lightning can strike through windows or travel through plumbing and wiring, though being inside is much safer than being outside.
- Tornadoes only happen in certain places (like "Tornado Alley"). Tornadoes can happen in many parts of the world, though they are more common in some areas.
- Opening windows during a tornado helps equalize pressure and protect a building. This is a myth -- it wastes time and does not help.

## Questions

```yaml
- question: "What should you do if you hear thunder and see lightning during a storm?"
  type: multiple-choice
  options:
    - "Stay outside and watch because lightning is interesting"
    - "Go inside a sturdy building and stay away from windows until the storm passes"
    - "Stand under a tree for shelter"
    - "Keep playing outside because thunder cannot hurt you"
  answer: 1
  explanation: "Lightning is dangerous and can strike people, trees, and open areas. The safest place is inside a sturdy building, away from windows. Trees are actually dangerous during lightning storms because lightning often strikes the tallest object in an area. Always go inside when you hear thunder -- if you can hear it, lightning is close enough to be dangerous."

- question: "A tornado is a spinning column of wind that reaches from a storm cloud down to the ground."
  type: true-false
  answer: true
  explanation: "A tornado is exactly this -- a violently rotating column of air that connects a thunderstorm cloud to the ground. The spinning winds can be extremely fast and powerful, which is why tornadoes can cause so much damage. If a rotating column does not reach the ground, it is called a funnel cloud, not a tornado."

- question: "Why is it important to listen to weather warnings on TV, radio, or a phone alert?"
  type: short-answer
  answer: "Weather warnings tell you that dangerous weather like thunderstorms, tornadoes, or hurricanes is coming to your area. They give you time to get to a safe place before the severe weather arrives. Without warnings, people might be caught outside or unprepared when a dangerous storm hits."
  explanation: "Weather warnings save lives because they give people time to take shelter. Weather scientists track storms with radar and satellites and send warnings so communities can prepare. Teaching children to take warnings seriously is one of the most practical outcomes of learning about weather."
```

## Explainer

Most weather is pleasant or at least harmless -- sunny days, gentle rain, a cool breeze. But sometimes weather turns **severe**, meaning it becomes strong enough to be dangerous. Learning about severe weather is not meant to scare you; it is meant to help you **stay safe** by knowing what to do.

**Thunderstorms** are storms that produce lightning and thunder. Lightning is a giant spark of electricity that jumps between clouds or from a cloud to the ground. It is extremely hot and powerful. Thunder is the sound that lightning makes -- the air around the lightning bolt heats up so fast that it explodes outward, creating a boom. If you see lightning or hear thunder, you should go inside a sturdy building right away. Never shelter under a tree, because lightning tends to strike tall objects.

**Tornadoes** are spinning columns of air that extend from a thunderstorm cloud down to the ground. The wind inside a tornado can spin incredibly fast -- fast enough to tear roofs off houses, flip cars, and uproot trees. Tornadoes usually form during powerful thunderstorms and can appear quickly. If there is a tornado warning, you should go to the lowest, most interior room of a building -- a basement if you have one, or an inside hallway or bathroom. Stay away from windows.

**Hurricanes** are enormous storms that form over warm ocean water. They can be hundreds of kilometers wide and bring heavy rain, powerful winds, and flooding that lasts for days. Unlike tornadoes, which form and disappear in minutes, hurricanes can last for a week or more and are tracked by weather scientists days before they arrive. People in the hurricane's path are warned ahead of time so they can prepare or evacuate. The most important lesson about severe weather is simple: **pay attention to warnings and get to a safe place**.
