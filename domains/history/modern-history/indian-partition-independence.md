---
id: indian-partition-independence
title: Indian Independence and the Partition
domain: history
course: modern-history
prerequisites:
- id: decolonization-independence-movements
  type: hard
- id: imperialism-asia-opium-wars
  type: soft
builds-toward:
- postcolonial-state-building-challenges
tags:
- India
- Pakistan
- independence
- partition
- Gandhi
- religious-conflict
stage: formal-systems
status: draft
---

# Indian Independence and the Partition

## Core Idea
In 1947, British India gained independence under Mohandas Gandhi's leadership but was simultaneously partitioned into secular India and Islamic Pakistan, creating one of history's largest forced migrations with over a million deaths in communal violence. The partition reflected disagreements between Hindu and Muslim nationalist visions and left lasting territorial and religious divisions. India emerged as a multiethnic, multi-religious democratic state; Pakistan as an Islamic republic. The partition illustrated both anticolonial nationalism's success and its dangers when religious identity became paramount in nation-building.

## Questions

```yaml
- question: "What was the core claim of Jinnah's Two-Nation Theory?"
  type: multiple-choice
  options:
    - "That India should be divided into many smaller states based on ethnicity and language"
    - "That Muslims and Hindus constituted distinct nations requiring separate political arrangements, not merely different communities within one state"
    - "That British India should remain unified under a federal system with strong protections for religious minorities"
    - "That Muslim-majority regions had been economically exploited and deserved autonomous governance"
  answer: 1
  explanation: "The Two-Nation Theory was a categorical claim: Muslims were not simply a religious minority within an Indian nation but a separate nation altogether, with distinct history, culture, and political identity. This meant that independence into a Hindu-majority India was not liberation for Muslims but the replacement of one form of domination with another. The theory provided the ideological basis for Pakistan's creation — not as autonomy within India but as a fully separate sovereign state."

- question: "Why was Mohandas Gandhi assassinated in January 1948?"
  type: multiple-choice
  options:
    - "By a Muslim extremist who blamed him for failing to prevent partition"
    - "By a British intelligence operative who feared his continued political influence"
    - "By a Hindu nationalist who blamed him for accommodating Muslim demands and enabling Pakistan's creation"
    - "By a Congress faction that disagreed with his strategy of non-violent resistance"
  answer: 2
  explanation: "Gandhi was shot by Nathuram Godse, a Hindu nationalist who viewed Gandhi's efforts at Hindu-Muslim reconciliation — particularly his insistence on treating Muslims fairly during partition's violence — as a betrayal of Hindu interests that enabled Pakistan. The assassination is deeply ironic: Gandhi had spent the summer of 1947 walking between villages trying to stop communal killing, and was killed precisely because he opposed the partition he is sometimes falsely credited with supporting."

- question: "Mohandas Gandhi supported the partition of British India as a pragmatic compromise necessary to achieve independence."
  type: true-false
  answer: false
  explanation: "Gandhi strongly opposed partition. He had spent decades working toward a unified, secular India where Hindus and Muslims could coexist. During the violence of 1947, he walked through Bengal attempting to stop communal massacres. His assassination by a Hindu nationalist who blamed him for Pakistan's creation is the sharpest evidence of his actual position: he was killed for being too sympathetic to Muslims, not for having supported division."

- question: "The Kashmir conflict originated in 1947 when a Hindu ruler of a majority-Muslim princely state chose to accede to India rather than Pakistan."
  type: true-false
  answer: true
  explanation: "Maharaja Hari Singh, the Hindu ruler of majority-Muslim Kashmir, acceded to India in October 1947 after Pakistani-backed forces entered the region. Pakistan contested this on the logic that Muslim-majority areas should have joined Pakistan under the same reasoning that created it. India maintained the accession was legally valid. Four wars later, the Line of Control still divides Kashmir, and the conflict remains one of the world's most dangerous unresolved territorial disputes — a direct inheritance of the partition's unresolved contradictions."

- question: "Explain how the Indian partition illustrated both the success and the dangers of anticolonial nationalism."
  type: short-answer
  answer: "The partition showed anticolonial nationalism's success: organized political pressure forced the British to withdraw from their largest colony. But it also showed the danger of nationalist movements built on exclusive religious identity. When 'the nation' was defined by religion, the people of the wrong religion within the new state became threats rather than fellow citizens. The result was catastrophic — 14–17 million displaced, up to 2 million killed — as communities on the wrong side of hastily drawn borders were attacked or expelled. Independence and partition were not separate events but the same event, showing that removing a colonial power does not automatically resolve the question of who belongs to the nation."
  explanation: "Anticolonial movements can unite diverse groups against a shared external enemy, but that unity often fractures when the enemy leaves and internal power must be distributed. India and Pakistan represent two answers to the nation-building question — secular pluralism versus religious nationalism — both born from the same moment of independence, shaped by the same colonial legacy, and still defined partly in opposition to each other."
```

## Explainer

You've already studied decolonization as a broad phenomenon — how European empires retreated after World War II, and the role of anticolonial nationalism in forcing that retreat. India's independence in 1947 is the most consequential single instance of that process, and also its most tragic, because it came simultaneously with partition — the creation of two states where the British had administered one, at a human cost that still shapes South Asian politics today.

The British had governed a subcontinent of extraordinary religious and ethnic diversity through a combination of direct rule, princely state alliances, and divide-and-rule administrative policies. The **Indian National Congress**, led by **Mohandas Gandhi** and **Jawaharlal Nehru**, sought independence for a unified, secular India. The **Muslim League**, led by **Muhammad Ali Jinnah**, argued that Muslims constituted a separate nation and would face permanent minority status in a Hindu-majority independent India. Jinnah's **Two-Nation Theory** — that Hindus and Muslims were not merely different communities but distinct nations requiring separate political arrangements — drove the demand for **Pakistan**.

The partition that resulted was catastrophic in its implementation. When the British transferred power in August 1947, the new borders cut through Punjab and Bengal, dividing communities, families, and infrastructure integrated for centuries. Approximately 14–17 million people were displaced — one of the largest forced migrations in history. In the violence that accompanied the transfer, somewhere between 200,000 and 2 million people were killed in communal massacres, with Hindus, Muslims, and Sikhs perpetrating and suffering atrocities. Gandhi, who had spent the summer walking between villages in Bengal trying to stop the killing, was assassinated in January 1948 by a Hindu nationalist who blamed him for Pakistan's creation.

The legacy is still live. India and Pakistan have fought four wars over **Kashmir**, a majority-Muslim princely state whose Hindu ruler acceded to India in 1947 — a decision neither Pakistan nor the Kashmiri independence movement accepted. The partition structured both countries' national identities in relation to each other: India defined itself partly as secular and plural in contrast to Pakistan's Islamic identity; Pakistan defined itself as the guardian of South Asian Muslim political interests. The boundary drawn hastily by British barrister Cyril Radcliffe in a matter of weeks remains one of the world's most contested and militarized borders.
