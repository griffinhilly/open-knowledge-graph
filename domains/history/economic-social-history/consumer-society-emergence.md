---
id: consumer-society-emergence
title: "Consumer Society: Mass Production, Advertising, and Consumption"
domain: history
course: economic-social-history
prerequisites:
- id: industrial-revolution-economic-impact
  type: hard
- id: urbanization-history
  type: soft

tags:
- history
- Economic Social History
stage: advanced
status: validated
---

# Consumer Society: Mass Production, Advertising, and Consumption

## Core Idea
Consumer society — organized around mass production and consumption of goods — emerged in the 20th century, particularly in the US. Mass production (assembly line manufacturing) created abundance of cheap goods; rising wages enabled workers to consume beyond subsistence; advertising created demand for goods. Shopping became a leisure activity; consumption became central to identity and aspiration. Advertising shifted from informing about products to creating desire and associating products with status and happiness. Consumption became increasingly central to how people defined themselves and found meaning — 'I shop, therefore I am.' Yet consumer society rests on contradiction: it requires continuous consumption growth, yet resources are finite. It also creates inequality: access to consumer goods depends on income; advertisements promote consumption as the path to happiness, yet consumption purchased with debt and low wages does not deliver happiness. Consumer society also generates waste: planned obsolescence ensures goods break or become unfashionable, requiring replacement. Understanding consumer society's history reveals it was actively created through advertising, credit systems, and corporate strategy — it was not inevitable that consumption would become central to social life and meaning-making. It also shows how consumption can be a form of social control: if people seek meaning and identity through consumption, they are vulnerable to advertising and spending, and less likely to seek meaning through political participation or community.

## Questions

```yaml

- question: "What technical and economic innovations made the emergence of consumer society possible in the early 20th century?"
  type: short-answer
  answer: "Mass production, pioneered by Henry Ford's assembly line (Model T production from 1908), dramatically reduced the cost of manufactured goods by substituting machinery and division of labor for skilled craftsmanship. The Model T fell in price from $850 in 1908 to $260 in 1925 as production scaled. Rising real wages (Ford paid $5/day -- roughly double the market wage -- to reduce turnover and enable his workers to buy his cars) meant workers had purchasing power beyond subsistence. Installment credit ('buy now, pay later') enabled purchases of durable goods beyond immediate income. Department stores created concentrated retail environments designed for browsing and impulse purchase. Advertising shifted from informing about products to creating aspirational desire -- the advertising industry grew tenfold in the 1920s."
  explanation: "The interplay of production, credit, wages, and advertising was essential to consumer society's emergence. None of these individually was sufficient -- all had to develop together. The 1920s US consumer economy was the model that spread, with variations, to other wealthy countries after WWII."

- question: "The planned obsolescence strategy was introduced by General Motors in the 1920s. What was it and what were its economic consequences?"
  type: multiple-choice
  options:
    - "GM planned to obsolete Ford's Model T by producing a better car, which was a standard competitive strategy with no unusual implications"
    - "GM's Alfred Sloan introduced annual model changes and a range of cars at different price points, creating psychological obsolescence -- the sense that last year's car was outdated -- to drive repeat purchases beyond functional need"
    - "GM planned to make cars wear out faster through inferior materials, ensuring customers would need replacements within 3-5 years"
    - "Planned obsolescence was introduced by the electronics industry in the 1960s; the automotive industry adopted it later"
  answer: 1
  explanation: "Alfred Sloan's strategic insight: Ford's Model T was durable and functionally adequate, so customers bought one and kept it. GM introduced annual styling changes ('the annual model') and a car ladder (Chevrolet to Pontiac to Oldsmobile to Buick to Cadillac) that made last year's model seem outdated. This was psychological rather than functional obsolescence -- the 1929 Chevrolet didn't stop working, but the 1930 model looked different and made the old one seem behind the times. This created continuous demand for new products even when old ones still functioned. Advertising amplified it: owning new things signaled status and modernity."

- question: "How did advertising's purpose and methods change between the 19th and 20th centuries?"
  type: short-answer
  answer: "19th-century advertising was primarily informational: it announced the existence and price of goods and described their features. Early newspaper ads for patent medicines and goods were text-heavy descriptions. The early 20th century saw advertising's transformation into desire creation: rather than informing, ads associated products with status, sex, freedom, and happiness. Edward Bernays (nephew of Sigmund Freud) applied psychoanalytic ideas to advertising, designing campaigns that connected products to unconscious desires. The 'Torches of Freedom' campaign (1929) associated women smoking with suffragist liberation, linking cigarettes to independence. Advertising became an industry of psychologists, designers, and copywriters whose explicit goal was creating emotional associations that bypassed rational evaluation. By the 1950s, TV advertising could reach millions simultaneously with carefully crafted emotional messaging."
  explanation: "Bernays's role in transforming advertising is documented in his own writing (Propaganda, 1928) and in Adam Curtis's documentary 'The Century of the Self.' The shift from information to desire creation is fundamental to understanding consumer culture's hold on 20th century life."

- question: "Consumer society in the 20th century raised living standards for most people in wealthy countries and therefore has no significant costs."
  type: true-false
  answer: false
  explanation: "Consumer society raised material living standards substantially and provided genuine benefits: access to goods (appliances, healthcare products, communications) that improved life quality. But it also generated documented costs. Environmental: continuous production growth depleted resources and generated waste and pollution. Social: advertising-driven consumption financed through debt created financial fragility (the 2008 crisis had roots in consumer debt); consumption became a substitute for other sources of meaning and community. Inequality: consumer goods access depends on income; advertising relentlessly promotes consumption as the route to happiness, which correlates with credit card debt and financial stress among lower-income consumers. Labor: global supply chains that produce cheap consumer goods often depend on poor labor conditions in producing countries."

- question: "What is the 'treadmill of production' argument, and how does it connect consumer society to environmental degradation?"
  type: short-answer
  answer: "The treadmill of production (Allan Schnaiberg, 1980) argues that capitalist economies have a structural tendency toward continuous growth: firms must grow to survive competition; workers need wages from growth; governments need tax revenue from growth. This treadmill generates continuous extraction of natural resources and disposal of waste products. Consumer society is the demand side of the treadmill: advertising creates desires for new products, planned obsolescence ensures existing products become obsolete, credit enables purchases beyond current income -- all driving continuous consumption growth. Environmental degradation is not accidental but structural: the same economic logic that generates growth also generates resource depletion, pollution, and waste. Addressing environmental problems thus requires addressing the growth imperative, not just technical fixes."
  explanation: "Schnaiberg's treadmill of production concept has been influential in environmental sociology. It frames environmental problems as systemic rather than the result of individual choices or bad actors -- a structural analysis that points toward different policy solutions than individual behavioral change."

```

## Explainer

Consumer society -- the economic and cultural system organized around mass production and continuous purchase of goods -- did not exist before the 20th century. Most people for most of history consumed at subsistence levels, buying only what they needed for immediate survival. The transition to a society where consumption became central to identity, aspiration, and daily life was engineered, not spontaneous.

The material preconditions assembled in the early 20th century. Henry Ford's moving assembly line (Highland Park plant, 1913) demonstrated that complex manufactured goods could be produced at radically reduced cost by substituting sequential machine operation for skilled craft work. The Model T fell from $850 in 1908 to $260 by 1925. Ford's "five-dollar day" (1914) -- roughly double market wages -- was simultaneously a labor retention strategy and a demand creation strategy: Ford wanted his workers to be able to afford his cars. This logic -- high wages enable consumption of mass-produced goods -- became the Fordist economic model that structured the postwar economy.

Mass production required mass consumption, which required two further developments: credit and desire. Installment credit ("buy now, pay later") was extended to consumer durables -- automobiles, furniture, appliances -- in the 1920s, enabling purchases beyond immediate income. By 1929, over 60% of automobiles and 80% of household appliances were purchased on installment plans. The advertising industry expanded tenfold in the 1920s, and its methods transformed. Where 19th-century advertising had informed, 20th-century advertising created desire -- using psychoanalytic insights (Edward Bernays, nephew of Freud) to connect products to unconscious aspirations. Smoking cigarettes was associated with women's liberation. Owning a new car was associated with modernity and social status. The product was the vehicle; the sold item was the feeling.

General Motors' Alfred Sloan contributed the crucial innovation of planned psychological obsolescence. Ford's Model T was durable and functional; customers bought one and kept it. Sloan's response was the annual model change and the car ladder (Chevrolet to Cadillac), creating the feeling that last year's model was outdated even when it still worked. This generated continuous demand for replacement beyond functional necessity. The strategy generalized across consumer goods: fashion in clothing, electronics cycles, kitchen appliance redesigns all follow the same logic.

The consumer society spread from the US to Western Europe and Japan after World War II, accelerated by the Marshall Plan's dollar transfers, postwar prosperity, and deliberate government policies to stimulate domestic consumption as Cold War political strategy. Suburban housing developments created demand for appliances, automobiles, and lawn equipment. Television brought advertising into living rooms with unprecedented reach.

Consumer society's contradictions are structural. It requires continuous growth of consumption to maintain employment and corporate profits; yet resources are finite and waste accumulates. It creates genuine prosperity; yet advertising systematically promotes consumption as the route to happiness, regardless of whether additional consumption produces it. It generates wealth; yet wealth concentrates at the production end while workers' wages in both rich and poor countries face downward pressure. These contradictions have generated environmental crisis, personal financial fragility, and cultural anxieties about meaning and sufficiency -- problems structural to consumer society's organization, not mere accidents.
