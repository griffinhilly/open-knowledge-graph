...
**15,285 topics** across 19 domains, **261 courses**. Radial graph shows 18 domains (practical-life-skills excluded). *(15,290 → 15,285 after the Jun-12 keystone edge-audit merged 5 grade-suffix duplicate topics.)* **+ 10 origin-layer `kind:capacity` nodes** (a private substrate, excluded from headline counts).

**Last session (Jul 1, 2026 — session 2: LEVEL 0 SHIPPED + LEVEL 1 BUILT, premortem-gated):** *(Concurrency with the wrapping session reconciled — this block supersedes its "nothing is pushed" note.)*
- **Level 0 PUSHED & DEPLOYED**: `43284aaf7..fccaae4ba` pushed to origin; Pages deploy SUCCESS (~26 min). First CI run with capacities in the corpus — live site now has the +3,044-edge builds-toward reconcile; capacities render nowhere on it (verified 0 refs in public radial pre-push).
- **Griffin's decision: "at least Level 0, probably Level 1"** → scoped premortem run (3 pessimists w/ bounce-rate / expert-reception / misreading lenses → 2 verifying optimists → 1 risk assessor, all Opus). **Verdict: CONDITIONAL GO** — full record: `plans/level1-premortem-2026-07-01.md`. Rulings: **§0.1 Observable Signs = NO public render** (spec §0 cond-1 already decided it; the scoped re-clear is satisfied for Level 1); renames independent, not gating (8/10 titles already plain). Biggest residual risk: caption-free screenshot semantics if the radial becomes the Jul-15 campaign hero — campaign use of the hub requires a baked-in caption + an explicit centerpiece decision.
- **Level 1 BUILT + VERIFIED + COMMITTED `b32b344aa` — UNPUSHED (push = the go-live decision, Griffin's call):** public radial now loads capacities; `--with-origins` demoted to INTERNAL review variant (full sections + private labeling). Public capacity panels allowlist `Core Idea`+`Emerges Through` (Observable Signs / On the Name never ship — grep-verified 0 hits); panel meta = "a capacity developed in early childhood"; Discernment 4×→3× + rings 12/24→16/28 (funnel-not-pinch, no occlusion); ancestry terminates AT capacities, full-chain button = "Show full chain to its origins"; titles: Symbolic Function→**Symbolic Thinking**, Grading and Seriation→**Ordering and Comparing** (slugs unchanged). **Headless-Chrome verified** (Playwright, 4 screenshots sent to Griffin): cold-load hub visible, no occlusion at zoom, public panel register correct, full-chain from linear-regression-ml lights 8 capacities, 0 JS errors. `validate.py --quick` PASSED; `test_fluency_capacity.js` ALL PASS.
- ~~Premortem condition still open: analytics beacon before ~Jul-15~~ **CLOSED Jul 2** — GoatCounter event beacon live (commit `d99745c7d`, events: `cap-panel-open/<slug>`, `topic-panel-open`, `ancestry-reveal`, `ancestry-full-chain`, `deeplink/{focus,ancestry,path}`; `no_onload` so CF stays the pageview source; dashboard https://okg.goatcounter.com). End-to-end smoke-tested on production (event request returned 200; the single `cap-panel-open/discernment-same-different` hit on the dashboard is the smoke test). Deploy fought through a GitHub Pages incident — 3 failed attempts (2× queue timeout during the incident, 1× the deploy-pages duplicate-artifact quirk: rerunning the job re-uploads the artifact; fix = DELETE the run's `github-pages` artifact via API before `gh run rerun --failed`).
- **Gotcha fixed en route:** local `output/js/graph.js` was stale from Jun 19 (pre-origin-layer) — it's written by `visualize_hierarchy.py build_graph_json()` (NOT `generate_quiz_page.py`); regenerate via `--index-only` before testing reveal features locally. CI always rebuilds it.
- **Observed discrepancy to verify:** A→B `?path=` bridge code (`revealPath`, `pendingPathFrom`, "Session 2" comment block) EXISTS in `visualize_radial.py` — item 3's "Session 2 NOT built" below is stale; verify the no-path-fallback UX against its hard gate before calling item 3 done.
- **RL-mastery findings unblocked for upstreaming** (the read-only guardrail existed for the uncommitted diff, now committed/pushed): F1/F6 soft→hard math-foundation edges on ~a dozen ML bridge topics, F2 orphan expert node. See `~/Projects/griffin/rl-mastery/findings.md`.

**Last session (Jul 1, 2026) — capacity DAG restructure + edge semantics + private-viz finalized + COMMITTED:**
- **Restructured the internal capacity DAG** (commit `fccaae4ba`): flipped naming↔symbolic-function (now `symbolic-function → naming` — general capacity precedes the specific skill); added `symbolic-function → discernment` (soft), `core-social → symbolic-function` (hard), `core-agents → core-social` (soft). Yields **two converging spines at the center hub** — physical/quantitative (`objects, number → discernment`) + symbolic/social (`agents → social → symbolic-function`); symbolic-function feeds both discernment & naming. Resolved the humanities-asymmetry (symbolic-function promoted to a central-tier feeder). `validate.py` PASSED (0 warnings, acyclic). Discernment Core Idea softened ("central" not "root"; names its feeders).
- **DURABLE edge-semantics decision** (`meta/schema.md`, "DAG as Pedagogical Simplification"): an edge `A → B` = "B **draws upon** A", NOT "A earlier in time"; `stage` is the separate chronological axis; the ~8% stage-inversion edges are legit under this reading; mutual reinforcement is never bidirectional (pick dominant direction). Applies graph-wide. Watch: don't let "sharpens" license near-cyclic edges.
- **Private radial FINALIZED** (`output/radial-with-origins.html`): capacity nodes white + black border (was gold), 2.5×; discernment enlarged to 4× as center hub; 3 feeders inner ring r≈12, other six outer r≈22; click-a-capacity shows inline body sections (no public page by design); white across ALL highlight/search/reveal paths (fixed a 5-path gold-flash bug class). Discernment settles at r≈3.9 (force sim), not dead-0. ⚠️ still browser-unverified visually (extension offline) — eyeball it.
- **Committed** `9defb0a51` (floor finish + rename + viz tools + earlier COMP updates) + `fccaae4ba` (restructure). *(PUSHED Jul 1 by session 2 as Level 0 — see the session-2 block above.)*
- **Next-session candidates (flagged, not done):** the truth-first faculty engine (`plans/capacity-spine-synthesis-2026-06-30.md`); 2 under-wired capacities — `core-objects` wired to only 5 topics, `core-agents` biology-dominant (14/20) — check whether specific psych/object-permanence pre-formal topics *should* carry them (fix by checking topics, NOT loosening regex).

**Prior session (Jun 30, 2026) — FINISH THE FLOOR + Discernment rename + private viz + capacity-spine ideate (committed Jul 1 in the two commits above):**
- **A′ wiring — floor extended 80 → 307 of 314 pre-formal topics** carrying a DIRECT capacity prereq (7 deliberately unfloored: 3 art/expression judgment-calls + 4 procedural/values; all 7 still reach a capacity transitively). Driven by a rewritten `tools/wire_capacities.py`: per-topic regex over **TITLE+TAGS only** (dropping course-name from the haystack — a course-name blanket match was stamping wrong capacities on 16 living-things topics, the disqualifier for naive "wire-all") + curated `COURSE_DEFAULTS` for homogeneous courses (feelings→core-social+symbolic-function, music→discernment, stories→symbolic-function) + homonym guards + a `reconcile_prereqs` step that ADDS **and REMOVES** capacity edges to match target exactly. Review file: `plans/origin-layer-wiring-review.md` (regenerated, 307 grouped by domain/course; Griffin eyeballed → "art judgment calls look correct"). Re-derive: `python tools/wire_capacities.py --report`.
- **A-vs-B tradeoff dialectic** (wire-all-314 vs wire-only-37-true-roots) → resolved to **A′ (corrected-A)**: scope was never the fault line, regex precision was. Both deciding beliefs landed on A′.
- **Connectivity invariant added to `tools/validate.py`**: every pre-formal `kind:topic` must reach a `kind:capacity` via prereq ancestry. Verified 314/314 reach live; 0/314 if capacities ignored (proves capacities are load-bearing, not decorative).
- **Rename "Discrimination" → "Discernment"** (culturally-loaded term). Surgical: global-replaced only the unique slug `discrimination-same-different` + the display word in OUR capacity node and code labels; did NOT touch unrelated legit uses (price-discrimination, item-discrimination, chord-discrimination). File renamed `discrimination-same-different.md` → `discernment-same-different.md`. Excluded a stale `.claude/worktrees/` snapshot.
- **Two PRIVATE origin-layer viz** (capacities stay internal-only; central hub, NO radial wedge — Griffin's decision): `tools/visualize_origin_layer.py` → `output/origin-layer-map.html` (standalone D3 force: 10 caps + 307 floored topics + 721 edges; load distribution: Discernment 307 / Number 82 / Grade 70 / Symbolic 70 / Social 44 / Space 41 / Naming 39 / Classification 33 / Agents 20 / Objects 5). And `tools/visualize_radial.py --with-origins` → `output/radial-with-origins.html`. **Public radial unchanged (0 capacities) — verified.** *(Layout + styling FINALIZED Jul 1 — see the Jul-1 block above. Both still verified STRUCTURALLY only; browser offline, eyeball before relying.)*
- **CAPACITY-SPINE ideate dialectic** (5 generators → 2 challengers → 3 synthesizers, all Opus) on extending the 10-capacity floor into a floor-to-ceiling "spine" of cognitive faculties (LOCKED: distinct more-abstract faculty-sets at altitude, not the same 10 re-weighted). Master list: `plans/capacity-spine-ideate-2026-06-30.md`. **Decision-ready synthesis: `plans/capacity-spine-synthesis-2026-06-30.md`** (BUILD/EXPLORE/PARK tiers, honesty cuts, the §0.1 ruling, the validation harness, engine spec).
  - **CRUX RESOLVED — Griffin chose TRUTH-FIRST**: build the faculty-profile engine (B1 propagation operator + B6 domain×thread prior + B9 keystone+altitude seeds, faculties OUTSIDE the graph) as a **PRIVATE truth-test** — *does the prereq graph encode real faculty structure, or am I imposing one?* — gated on **3 pre-registered validation gates** (G1 face-validity hold-out, G2 topology-recovers-the-hand-authored-basis via factorization, G3 faculty⊥domain discriminant) BEFORE any user-facing surface ships. Value is independent of traffic; matches the discovery-over-invention ethic.
  - **Track A cheap shareables ship in parallel (no decision needed):** D1 plain-language renames of the 10 capacities, ONE static "Cradle-to-Frontier" poster (real prerequisite-lineage paths, NOT faculty-continuity claims), an "orphan faculties" honesty post, codify anti-checklist + bounce-trap rules as spec law, instrument privacy-safe analytics.
  - **Binding honesty cuts:** sell prerequisite LINEAGE not faculty CONTINUITY; kill the "your weak spot is faculty X" diagnostic (statistically unidentified); drop the "same faculty 30 years apart" hook (contradicts the locked layered decision).
  - **§0.1 ruling:** parent-facing *rendered developmental signs* (Wonder Cards) still need a SCOPED premortem re-clear next session — renames + the static poster don't render signs, so they ship freely.

**Last session (Jun 26, 2026) — ORIGIN LAYER pilot (probe → dialectic → premortem → build):** Triggered by Griffin's observation that "binaries" (hot/cold) and other developmental precursors were absent. Decided via a full process chain (all artifacts under `plans/`): research probe (`precursor-primitives-probe-2026-06-26.md` — 4 frameworks, convergence CONFIRMED) → `--tradeoff` dialectic (curriculum-map vs learnability-map; verdict: typed sub-pre-formal node-type, DECOUPLED from any tracker, "hourglass/center" claim DROPPED — it's a funnel not a pinch) → `--premortem` (CONDITIONAL GO; reversed the earlier "render pages" decision to GRAPH-ONLY; caught a real fluency-cap regression). **SHIPPED (graph-only pilot):** new `kind: capacity` field + `proto-formal` stage + `reference` status + `developmental-origins` hidden meta-domain; **10 capacity nodes** (5 core systems objects/agents/number/space/social + discrimination → grade-seriation → naming-symbol-reference → classification-sorting → symbolic-function gateway); "binaries" live on `naming-symbol-reference`. Capacities stay IN the path-graph (ancestry now resolves — fixes the 314-disconnected-roots dead-end) but excluded from EVERY public surface (pages, keystone, JSON-LD, radial, domain maps, sitemap, headline counts) via `kind`-guards. **80 pre-formal roots wired** (kindergarten + feelings) via deterministic `tools/wire_capacities.py`; anti-collapse gate PASSES (10 signatures, top 32%). Fluency: capacities are assumed-known (pruned from adult paths, never cap successors — `tools/test_fluency_capacity.js` regression test green). `validate.py` clean; `## Observable Signs` non-diagnostic lint in CI. Spec + conditions: `plans/origin-layer-spec.md`. **Identity:** "everything learnable" kept only as an aspirational North Star, NOT a public claim. **PENDING:** ~~(a) Griffin's condition-4d eyeball of ~20 wirings~~ **DONE Jun 30** (eyeballed `origin-layer-wiring-review.md`, "art judgment calls look correct"); ~~(b) ~234 remaining pre-formal roots un-wired~~ **DONE Jun 30** (floor extended to 307/314 via A′); ~~(c) full CI regen + commit~~ **COMMITTED Jul 1** as `9defb0a51` (per `plans/commit-plan-origin-layer.md`) + `fccaae4ba` (restructure). Unpushed (master 4 ahead of origin). The 2 dirty `tools/overnight/` scratch files remain correctly unstaged.

**Last session (Jun 13–17, 2026) — distribution BUILD sprint (items 1–3 shipped):** All live + verified on production. (1) Analytics beacon, (2) B3 og:image cards + A6 topic-page upgrades, (3) keystone leaderboard (`keystones.html`, scoring = Griffin's 3 principles, edge-audit gate satisfied → 155 corpus fixes + 5 merges), (4) **path engine Session 1** (`visualize_radial.py`: ancestry reveal hop-capped@4 + "show full chain", search-first auto-reveal, `?ancestry=` deep link + copy-link; spine = `centerOnNode`/dim-overlay/`withFullGraph` lazy-load). Strategy fork RESOLVED (distribution primary; parent-acq parked → Jul-15 routine). **Jun 17 tangents:** Search Console `/index.html` "Alternate page with proper canonical tag" diagnosed BENIGN (canonical→`/` which is indexed; no action); landing-page record-check — "radial as landing page" was NEVER a locked decision (research found graph-as-hero is a documented failure mode; search-first was the rec) — Griffin chose to hold to Fable's path-engine plan, NOT deviate. **Khan-granularity finding** (parked backlog): scraped Khan's archived 2019 math map (1,976 skills via embedded-JSON, `/tmp/ka_math_skills.json`) — near-equal total to OKG (1,971) but Khan is ~2.6× denser in K-12+intro-calc (1,976 vs OKG's 754 in-band) because Khan indexes *exercises*, OKG indexes *concepts*; OKG spends its count on graduate breadth Khan lacks. Open question: a sub-area concept-alignment (e.g. 3rd–4th grade arithmetic) to find where Khan's finer splits are prereq-distinct concepts OKG is bundling. Offered, not run.
...
**Known issues:**
- **Standing corpus debt — ~12,122 validator warnings (surfaced by Jun-17 reviewer; previously invisible/un-owned).** `validate.py` PASSES (0 errors, 0 cycles) but on top of ~9,331 bidirectional-prereq mismatches + ~1,125 dangling "not found" references + the 90-file duplicate-prereq-id bug (only 2 fixed in the keystone scope). The Jun-12 edge-audit moved the total by only ~8 (it correctly targeted edge *correctness* on top keystone candidates, not global debt). NOT blocking, but "validate.py clean" overstates — it's clean-of-errors, not clean-of-warnings. Needs an owner + a triage plan before it's cited as quality evidence.
...
0. **Strategy fork RESOLVED (Jun 12, Griffin's call): distribution track is primary; parent-acquisition track PARKED with a ~mid-July 2026 re-entry trigger** (prep for an August back-to-school push). Rationale, recorded so future sessions don't re-litigate: the 7-week parent-track pause was partly *seasonal strategy*, not just attention drift — pushing an educational resource at parents in the closing months of a school year felt futile. School resumption (~August) is the natural attention window; the summer is for building the attention-grabbing tools that augment kids' regular learning materials, then "make moves to get the flywheel going starting in August." **Analytics SHIPPED Jun 12** (CF Web Analytics beacon in `seo_meta_tags()` + direct inserts on tag/assessment pages; manual-install token, CF-side auto-injection OFF to avoid double-counting). Verify events appear in the CF dashboard ~Jun 13.
1. ~~**B3 og:image cards + A6 topic-page upgrades**~~ **BUILT Jun 12** — `tools/render_og_cards.py` (playwright/Chrome, top-1000 hubs by transitive successors + default card, CI step before topic pages); og:image/twitter:card via `seo_meta_tags(image=)`; A6 = site-context line, hero stats ("N topics build on this" + map button), neighbor strip (prereqs → hue-dot → successors), caret→`<sup>` cleanup (1,617 files affected); minimal `?focus=<id>` deep link on radial (camera+select+panel — full ancestry highlight still belongs to the path-engine item); `validate_seo.py` now requires og:image + existing card file on every page. Visually verified (cards, page desktop/mobile, focus link).
2. ~~**Keystone leaderboard**~~ **BUILT Jun 12-13, gate SATISFIED** — methodology locked by Griffin (his 3 principles: cross-domain reach ×2 bonus, 0.6/hop decay over 6 hops, within-stage ranking; supersedes raw/cap/greedy options); edge-audit run on top-15-per-stage (23 agents, 1,464 edges, 10.1% flagged → 155 fixes applied + 5 dup merges, commit f48cf26c0); `tools/generate_keystone_page.py` → `output/keystones.html` (5 stage bands summing to 50, advanced+expert folded into "Graduate & Beyond" [expert band honestly thin pending the ~60 missing expert topic-areas], methodology + GitHub-issue flywheel link on page, "reaches X topics across Y domains" framing); og/keystones.png share card; sitemap + validate_seo + CI + index-hero link wired. Verified desktop/mobile/0 JS errors.
3. **Path engine cluster** — ONE engine for A3 ancestry reveal → A2 search-first → B4 shareable subgraphs → curated six-degrees gallery. HARD GATE: explicit no-path fallback UX before any two-topic surface ships.
   - **Session 1 BUILT Jun 13** (`tools/visualize_radial.py`, plan `~/.claude/plans/fancy-nibbling-robin.md`): engine spine + all one-topic surfaces. **A3** ancestry reveal — click/search a topic → dim graph, light its prereq tree; **hop-capped at 4** (Griffin's call) with banner "Show full chain (N)" expansion; structural backward-BFS over the full graph (no mastery filter). **A2** — single-match search now centres + auto-reveals ("What do you want to understand?" placeholder). **B4** — `?ancestry=<id>` deep link + panel "Copy link". Spine: `centerOnNode()` (extracted from the `?focus=` dup, now with off-centre offset so the panel doesn't cover the fan), `pathNodeSet`/`pathEdges` dim+overlay render mode in `draw()`, `withFullGraph()` lazy-load of `js/graph.js` (3.5MB stays OFF the cold path — verified 0 requests until first reveal), `#pathBanner` status/clear UI, Esc-to-clear. Foundational topics → "starting point, nothing comes before it" (always-answerable). **Edge type not retained in graph.js `p`/`s`**, so ancestry edges render uniformly (not hard/soft-styled). Verified headless: cold-load gating, search→reveal, full-chain expand 15→254, Esc clear, foundational, deep-link, desktop+mobile, 0 path-engine JS errors.
   - **Session 2 (NOT built, gate binds here)**: A→B bridge path (directional prereq chain when one topic is an ancestor of the other) + **no-path fallback UX** (most arbitrary pairs don't connect — show each topic's own foundations / nearest shared ancestor, stated explicitly; *the fallback IS the feature*) + `?path=<a>,<b>` URLs + curated edge-audited counterintuitive-pairs gallery. Reuse the Session-1 spine (`centerOnNode`, dim/overlay, `withFullGraph`, banner). `worldToScreen` deferred to here (unused in S1).
...
- **Keystone edge-audit follow-ups (Jun 12, post-audit; none gate the leaderboard)**: (a) **systemic sweep — electrodynamics course is staged `expert` wholesale** (verified by grep; standard undergrad E&M → formal-systems; only the audited candidates got fixed); (b) **systemic sweep — early-grade course/stage mismatches** (8 audited 1st-grade topics were `pre-formal`; run `tools/audit_course_stages.py` over K-2 courses for the rest); (c) **validator rule: reject duplicate prereq ids in one file** (audit found same id listed twice with conflicting hard/soft in ≥3 files); (d) **dup investigations beyond the 5 merged pairs**: metric-spaces-definition{,-and-examples,-examples} triple, topological-spaces same triple, electronegativity-and-bond-polarity vs -polarity-continuum, set-theory-fundamentals vs set-theory-basics, one-to-one-correspondence vs one-to-one-counting-correspondence, counting-to-10 vs counting-sequence-one-to-ten, wave-equation-pde vs wave-equation-one-dimensional, krebs-cycle vs citric-acid-cycle-mechanism (possibly legit two-level treatment); (e) 19 UNSURE edges parked in `output/_keystone_audit/AUDIT_REPORT.md` §"second look".
...
- **Scheduled cloud routines (manage at claude.ai/code/routines)**: Node-24 deploy check (one-time, Jun 17); SEO pulse (one-time, Jun 26); **parent-acquisition re-entry (one-time, Jul 15 @ 9am CT — surfaces the 2-day Twitter test gate decision-ready; created Jun 12 at fork resolution)**; **dormancy pulse (monthly, 1st @ 9am CT — nudges with PLAN item-0 verbatim if repo >21 days quiet; self-expires in-prompt after Sep 15, 2026, then Griffin disables it in the UI)**.
- Full dialectic verdict + EXPLORE/PARK tiers: `plans/ideate-dialectic-2026-06-11.md`.

**Next steps — PARENT-ACQUISITION TRACK (PARKED Jun 12, 2026 → re-entry trigger ~mid-July 2026 for the August back-to-school push):**
*Jun 12 fork decision: distribution is primary; this track resumes near school resumption. The May 23 kill-gate date in item 2 is superseded by the July re-entry trigger (it passed unexecuted — the reviewer's predicted failure mode — which is part of why the trigger must be externalized this time, not left to session memory).*
