...
**13,977 topics** across 19 domains, **229 courses**. Radial graph shows 18 domains (practical-life-skills excluded).

**Last session (Mar 29–30, 2026) — Stage calibration + P0/P1 expansion:**
- *Stage calibration*: 1,446 topics restaged via 14-agent swarm. Expert: 2,362→1,268. Advanced: 1,741→2,951. Chem/CS got expert for first time. Health/engineering/psychology heavily demoted.
- *P0 courses (10)*: algebraic-topology, PDEs, differential-geometry, ML-theory, cryptography, quantum-computing, QFT, general-relativity, condensed-matter, inorganic-chemistry (+310 topics)
- *P1 courses (22)*: commutative-algebra, stochastic-processes, representation-theory, information-theory, advanced-algorithms, formal-methods, nonlinear-dynamics, particle-physics, materials-chemistry, genomics, systems-bio, developmental-bio, structural-bio, remote-sensing, geochemistry, biostatistics, health-economics, IO-psychology, behavioral-economics, labor-economics, demography, continental-philosophy (+515 topics)
- *Radial*: Removed practical-life-skills from radial graph (kept on index). Swapped economics adjacent to history.
- *Tooling*: Built `find_near_duplicates.py`, fixed counting-to-ten dupe

**Known issues:**
- **3 P1 courses may be incomplete** (rate-limited mid-generation): commutative-algebra (13/25), representation-theory (16/25), stochastic-processes (21/25). Need top-up.
- **825 new topics have no quality review** — Q+E generated by agents, not verified. Need spot-check.
- **_domain.yml course stages inconsistent** with actual topic stages for many courses. Automated reconciliation reverted (min-stage logic wrong). Needs manual fix — may affect domain map tier layout.
- **8 near-duplicate pairs** identified by find_near_duplicates.py (normalized matches). Plus philosophy-of-science ~10-12 duplicates.
...
**Next steps:**
1. **Verify new content quality** — spot-check 5-10% of 825 new topics for stage-appropriate complexity, accurate Q+E, no duplicates of existing content
2. **Complete incomplete P1 courses** — top up commutative-algebra (+12), representation-theory (+9), stochastic-processes (+4)
3. **Reconcile _domain.yml** — fix course stages to match actual topic stage distributions (needs smarter logic than min-stage)
4. **P2 topic generation** (~550-715 topics, 6 new courses + ~50 extensions to existing courses)
5. **P1 extensions** (~80-105 topics: set-theory, classical-mechanics, organic-chemistry, epigenetics, pop gen, biochemistry, volcanology)
6. **T/F semantic spot-check** + 79 merge-conflict topics
7. **Phase 9D**: Landing page redesign, domain toggle on radial, progress bars, guided learning paths
8. Write announcement post
